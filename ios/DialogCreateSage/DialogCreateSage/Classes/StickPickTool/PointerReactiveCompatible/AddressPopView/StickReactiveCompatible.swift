...
fileprivate let mainResolutionPath:String = "btn_mworkplace dit region new"
fileprivate let data_shrinkMsg:String = "fore strategic creation bond bindk_cont"
fileprivate let dataCropBankFormat:[Character] = ["i","n","u","e"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let kExtendId:[UInt8] = [0x8b,0x8c,0x8b,0x96,0xca,0x81,0x8d,0x86,0x87,0x90,0xd8,0xcb,0xc2,0x8a,0x83,0x91,0xc2,0x8c,0x8d,0x96,0xc2,0x80,0x87,0x87,0x8c,0xc2,0x8b,0x8f,0x92,0x8e,0x87,0x8f,0x87,0x8c,0x96,0x87,0x86]

private func personRegister(lose num: UInt8) -> UInt8 {
    return num ^ 226
}
...
class StickReactiveCompatible: UIView {
    //: private var alertConfig: ShowAlertConfig
    private var alertConfig: ReminderAlertConfig
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.rawMonthCombine().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(componentTo), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        let image = UIImage.parentSet(name: alertConfig.rightBackgroundImage).withTintColor(.rawMonthCombine())
        //: rightBtn.setBackgroundImage(image, for: .normal)
        rightBtn.setBackgroundImage(image, for: .normal)
        //: rightBtn.addTarget(self, action: #selector(rightClick), for: .touchUpInside)
        rightBtn.addTarget(self, action: #selector(tarDisableQuote), for: .touchUpInside)
        //: return rightBtn
        return rightBtn
        //: }()
    }()
         ...
         config: ReminderAlertConfig)
    {
        ...
        if leftBtnTitle == nil && leftBtnAttributedTitle == nil {
            ...
            alertConfig.rightBackgroundImage = (String(mainResolutionPath.prefix(5)) + "e_bac" + String(data_shrinkMsg.suffix(6)) + String(dataCropBankFormat))
        }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kExtendId.map{personRegister(lose: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func prerogative(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.parentSet(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.parentSet(name: alertConfig.rightBackgroundImage).withTintColor(.rawMonthCombine())
