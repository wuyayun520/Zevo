...
/*: "Select a date" :*/
fileprivate let show_stallTitle:[Character] = ["S","e","l","e","c","t"," ","a"," ","d","a","t","e"]
...
protocol OverObjectProtocol: NSObjectProtocol {
...
class RoundDataSource: PickWeeklyThen, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: OverObjectProtocol?
    ...
    override func numbererest() {
        ...
        titleString = (String(show_stallTitle)).localized
        ...
        moment(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func moment(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func everyEquity(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.nor(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.nor(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return enterUpon(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func enterUpon(year: Int, month: Int) -> Int {
