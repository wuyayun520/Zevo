...
fileprivate let app_extentThirdId:[UInt8] = [0x49,0x4e,0x49,0x54,0x8,0x43,0x4f,0x44,0x45,0x52,0x1a,0x9,0x0,0x48,0x41,0x53,0x0,0x4e,0x4f,0x54,0x0,0x42,0x45,0x45,0x4e,0x0,0x49,0x4d,0x50,0x4c,0x45,0x4d,0x45,0x4e,0x54,0x45,0x44]

private func yesterdaySouth(party num: UInt8) -> UInt8 {
    return num ^ 32
}

/*: "FFD855" :*/
fileprivate let userCounteractionMsg:String = "purchaseD855"
...
// __CLOSE_PRINT__
//
//  PremonitionReactiveCompatible.swift
//  AbroadTalking
//
...
class PremonitionReactiveCompatible: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - const_limitMsg
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: const_limitMsg,
                ...
                self.assessmentForLaunch(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_extentThirdId.map{yesterdaySouth(party: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (userCounteractionMsg.replacingOccurrences(of: "purchase", with: "FF")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: app_robotPathClickId, y: 0, width: const_limitMsg, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - const_limitMsg, y: 0, width: const_limitMsg, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.pushDown(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: app_robotPathClickId, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.pushDown(0, 0, 0, 0.6)
...
extension PremonitionReactiveCompatible {
    /// 重置
    //: func resetOriginalState() {
    func sendState() {
        //: self.leftSilder.frame = CGRect(x: LeftMargin, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.leftSilder.frame = CGRect(x: app_robotPathClickId, y: 0, width: const_limitMsg, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - const_limitMsg, y: 0, width: const_limitMsg, height: self.frame.size.height)
        //: resetIndicator()
        switcheCrop()
    }

    //: func needShowIndicator(isShow: Bool) {
    func requireShowBusinessIndexExhibit(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func assessmentForLaunch(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - const_limitMsg,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - const_limitMsg)
    }
}
...
extension PremonitionReactiveCompatible {
    ...
    @objc func purchaseGesture(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.assessmentForLaunch(startX: videoStartX)
    ...
    @objc func askGesture(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension PremonitionReactiveCompatible {
    ...
    private func itsyBitsy() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(purchaseGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(askGesture(gesture:)))
    ...
    func lineXposition(_ xposition: CGFloat) {
