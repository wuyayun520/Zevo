...
@interface PrettyData : NSObject

@end

@implementation PrettyData

+ (NSData *)PrettyDataToData:(NSString *)value {
...
+ (Byte *)PrettyDataToByte:(StructPrettyData *)data {
    for (int i = 0; i < data->step; i++) {
        data->laden[i] ^= data->finish;
    }
    data->laden[data->step] = 0;
	if (data->step >= 3) {
		data->accessBoldTier = data->laden[0];
		data->hitch = data->laden[1];
		data->succeed = data->laden[2];
	}
    return data->laden;
}

+ (NSString *)StringFromPrettyData:(StructPrettyData *)data {
    return [NSString stringWithUTF8String:(char *)[self PrettyDataToByte:data]];
}

//: 请打开相机权限
+ (NSString *)appPersonStr {
    /* static */ NSString *appPersonStr = nil;
    if (!appPersonStr) {
		NSString *origin = @"2F6870214E54227B47205C7F215B7D215A442E5E57DE";
		NSData *data = [PrettyData PrettyDataToData:origin];
        StructPrettyData value = (StructPrettyData){199, (Byte *)data.bytes, 21, 230, 75, 184};
        appPersonStr = [self StringFromPrettyData:&value];
    }
    return appPersonStr;
}

//: Width
+ (NSString *)const_dayFormat {
    /* static */ NSString *const_dayFormat = nil;
    if (!const_dayFormat) {
		NSString *origin = @"556B66766A0E";
		NSData *data = [PrettyData PrettyDataToData:origin];
        StructPrettyData value = (StructPrettyData){2, (Byte *)data.bytes, 5, 117, 236, 37};
        const_dayFormat = [self StringFromPrettyData:&value];
    }
    return const_dayFormat;
}

//: Height
+ (NSString *)app_retName {
    /* static */ NSString *app_retName = nil;
    if (!app_retName) {
		NSString *origin = @"AC818D838C90F5";
		NSData *data = [PrettyData PrettyDataToData:origin];
        StructPrettyData value = (StructPrettyData){228, (Byte *)data.bytes, 6, 222, 149, 185};
        app_retName = [self StringFromPrettyData:&value];
    }
    return app_retName;
}
...
#import "PunishAuthorize.h"
//: #import <UIKit/UIKit.h>
#import <UIKit/UIKit.h>

//: @interface STFilterCamera () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
@interface PunishAuthorize () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@implementation PunishAuthorize
                         ...
                         modify:(BOOL)needYuvOutput
{
    ...
    if (self) {
        ...
        self.videoDevice = [self userPosition:iDevicePosition];
...
- (void)blockFrame:(CGPoint)point position:(CGRect)frame
{
    ...
    [self limit:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           point:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                spacing:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)limit:(AVCaptureFocusMode)focusMode point:(AVCaptureExposureMode)exposureMode spacing:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self inner:^(AVCaptureDevice *captureDevice) {
...
- (void)inner:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self userPosition:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self storage]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:[PrettyData const_dayFormat]] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:[PrettyData app_retName]] floatValue];
...
- (CGRect)scaleReadFit:(CGRect)rect suit:(BOOL)bScaleToFit
{
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:[PrettyData const_dayFormat]] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:[PrettyData app_retName]] floatValue];
...
- (BOOL)storage
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:[PrettyData appPersonStr]
...
- (AVCaptureDevice *)userPosition:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(identity:mainBy:without:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate identity:captureOutput mainBy:sampleBuffer without:connection];
        }
    }
