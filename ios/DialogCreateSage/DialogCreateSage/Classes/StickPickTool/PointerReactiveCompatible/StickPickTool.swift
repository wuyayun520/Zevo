...
/*: "Finish" :*/
fileprivate let notiUpAdviceMsg:String = "area additional maker destinationFinish"

/*: "Cancel" :*/
fileprivate let app_houseUrl:[Character] = ["C"]
fileprivate let kAreaValue:[Character] = ["a","n","c","e","l"]

/*: "btn_photo_choice_pre" :*/
fileprivate let showIndividualUrl:[Character] = ["b","t","n","_","p","h","o","t","o","_","c","h","o","i","c","e","_","p","r","e"]

/*: "btn_photo_add_nor" :*/
fileprivate let k_mutualStr:String = "btn_info slim service ad position"
fileprivate let show_resolutionName:String = "it reward rightadd_n"
fileprivate let appOffdData:String = "ohealthy"

/*: "/tmp" :*/
fileprivate let show_customerMsg:String = "south request enter local/tmp"

/*: "Export session failed" :*/
fileprivate let data_platStr:[Character] = ["E","x","p","o","r","t"," ","s","e"]
fileprivate let dataSameLeaveMessage:[Character] = ["s"]
fileprivate let const_foodData:[Character] = ["s","i","o","n"," ","f","a","i","l","e","d"]

/*: "Export canceled" :*/
fileprivate let data_stepName:String = "Exporrequire quick sphere go var"
fileprivate let noti_remainData:[Character] = ["t"," ","c","a","n","c","e","l","e","d"]

/*: "Successful!" :*/
fileprivate let userWeatherValue:String = "sink momentSucces"

/*: "Unsupported video formats" :*/
fileprivate let show_backgroundStateFemaleId:[Character] = ["U","n","s","u","p","p","o","r","t","e","d"," ","v"]
fileprivate let noti_worldTitle:[Character] = ["i","d","e","o"," ","f","o","r","m","a","t","s"]

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let notiPoliceUrl:String = "wealthy tip retain two locationyyyy-"
fileprivate let show_gainTitle:String = "HH:msuccess me admin extent"
fileprivate let dataLocalValue:String = "apply"

/*: "/tmp/video- :*/
fileprivate let dataInviteUrl:[Character] = ["/"]
fileprivate let showHideData:String = "tmp/vrecording lens violation"

/*: .mp4" :*/
fileprivate let k_yaFormat:[Character] = [".","m","p","4"]
...
class StickPickTool: NSObject {
    ...
    class func beyondVideo(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = user_needValue * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = user_needValue
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (kStreamTitle - height) / 2, width: user_needValue, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(notiUpAdviceMsg.suffix(6))).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(app_houseUrl) + String(kAreaValue)).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.processAcross(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.submaxilla()
        ...
        imagePicker?.naviTitleFont = UIFont.processAcross(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.antiquityColor()
        ...
        imagePicker?.iconThemeColor = UIColor.rawMonthCombine()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.parentSet(name: (String(showIndividualUrl)))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.parentSet(name: (String(k_mutualStr.prefix(4)) + "photo_" + String(show_resolutionName.suffix(5)) + appOffdData.replacingOccurrences(of: "healthy", with: "r")))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.processAcross(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(notiUpAdviceMsg.suffix(6))).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(notiUpAdviceMsg.suffix(6))).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.rawMonthCombine()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: user_needValue - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func displayEnable(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(show_customerMsg.suffix(4))))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(show_customerMsg.suffix(4)))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(data_platStr) + String(dataSameLeaveMessage) + String(const_foodData)))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(data_stepName.prefix(5)) + String(noti_remainData)))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(userWeatherValue.suffix(6)) + "sful!"))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(show_backgroundStateFemaleId) + String(noti_worldTitle)))
            }
        }
    }
    ...
    private class func signalOutput() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (String(notiPoliceUrl.suffix(5)) + "MM-dd-" + String(show_gainTitle.prefix(4)) + "m:ss-SS" + dataLocalValue.replacingOccurrences(of: "apply", with: "S"))
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(dataInviteUrl) + String(showHideData.prefix(5)) + "ideo-") + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(k_yaFormat)))
        //: return outputPath
        return outputPath
    }
}
