...
/*: "http://stat. :*/
fileprivate let const_thenTitle:String = "HTTP"

/*: .com" :*/
fileprivate let data_priorityStr:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let notiDebateFieldValue:String = "http://smooth task"

/*: "https://" :*/
fileprivate let dataShareFillCommitTitle:String = "hmakeps"
fileprivate let main_equalMarkTitle:[Character] = [":","/","/"]

/*: "http://m. :*/
fileprivate let const_openingFormat:[Character] = ["h","t","t","p",":","/","/","m","."]

/*: "http:// :*/
fileprivate let showLaboratoryLabelKey:String = "http:/support trust never sink"
fileprivate let constEqualUnderValue:[Character] = ["/"]

/*: "3,90" :*/
fileprivate let const_coalGoAccuracyName:String = "3,9tun"

/*: "1990-01-01" :*/
fileprivate let userPersonalityStr:[Character] = ["1","9","9","0","-","0","1"]
fileprivate let kMmTitle:[Character] = ["-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let k_faultMsg:[Character] = ["M","M","-","d","d","-","y"]
fileprivate let mainPublishCosStr:String = "exceptexceptexcept"
...
class RoundReactiveCompatible: NSObject {
    ...
    override init() {
        ...
        self.requestServer = CosSweetReactiveCompatible.share.serverUrlStr
...
struct MisestimationModelType: HandyJSON {
    //: var errno: Int!
...
public struct GiftWarningErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct SweetMTheoryTransformable: HandyJSON {
    ...
    var CSConfig = MaximumMeasurable() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (const_thenTitle.lowercased() + "://stat.") + "\(userDeviceIndexStr)" + (String(data_priorityStr))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(notiDebateFieldValue.prefix(7)))) && !statDomain.hasPrefix((dataShareFillCommitTitle.replacingOccurrences(of: "make", with: "tt") + String(main_equalMarkTitle))) {
                //: return "http://".appending(statDomain)
                return (String(notiDebateFieldValue.prefix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(const_openingFormat)) + "\(userDeviceIndexStr)" + (String(data_priorityStr))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(showLaboratoryLabelKey.prefix(6)) + String(constEqualUnderValue)) + "\(urlDomain)"
        }
    }
...
public class AfterConfigModel: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [TensionModelType] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [TensionModelType] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [TensionModelType] = [] // 语音收费设置
    ...
    var vipSetting = [AggregationSettingModel]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [HostBannerModel]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = RaceFinishModelType() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class OptReactiveCompatible: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [ListenModelType]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [ListenModelType]?
    ...
    var gallery: [RoundGreatTransformable]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (const_coalGoAccuracyName.replacingOccurrences(of: "tun", with: "0")) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.frankOpend(self.birthday ?? (String(userPersonalityStr) + String(kMmTitle)), dateFormat: (String(k_faultMsg) + mainPublishCosStr.replacingOccurrences(of: "except", with: "y")))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.doingSuit().year else { return "0" }
...
struct RoundGreatTransformable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct ListenModelType: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func need(compareTo: ListenModelType) -> Bool {
        //: return
        return
...
struct TwoMeasurable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [ExperienceMeasurable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [ExperienceMeasurable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct ExperienceMeasurable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [ListenModelType]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct MaximumMeasurable: HandyJSON {
...
struct AggregationSettingModel: HandyJSON {
...
struct RaceFinishModelType: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
