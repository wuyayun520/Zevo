...
/*: "add_header_cookies" :*/
fileprivate let main_botOverlookMessage:String = "formal child ionadd_heade"
fileprivate let data_waitPileStr:String = "r_cooon eff first food visual"
...
public extension UIImageView {
    ...
    @objc func agreeFinish(urlStr: String,
    ...
    internal func funReply(urlStr: String,
                           //: placeImg: UIImage = UIImage.placeImgSquare(),
                           placeImg: UIImage = UIImage.ground(),
    ...
    @objc func pastEstimated(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: userDismissUrl.allTitle(), with: userDeviceIndexStr)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.sessionImage())
    }
    ...
    @objc func dotRidIdentity(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: CosSweetReactiveCompatible.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(main_botOverlookMessage.suffix(9)) + String(data_waitPileStr.prefix(5)) + "kies"))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.ground(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
