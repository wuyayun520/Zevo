...
// __CLOSE_PRINT__
//
//  UIImage+MiscalculationCompress.m
//
...
@implementation UIImage (MiscalculationCompress)

//: - (NSData *)compressedImageData {
- (NSData *)availableData {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.process;
...
- (UIImage *)bit {
    ...
    return [UIImage imageWithData:[self availableData]];
}


//: - (UIImage *)wcSessionCompress {
- (UIImage *)lockCapacity {
...
- (UIImage *)instance:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self frame:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self estimated:size];
...
- (CGSize)frame:(Boolean)isSession {
...
+ (NSData *)acquire:(UIImage *)image background:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image resolution:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG noticeDown:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG cut:scalesize receive:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG noticeDown:(fileSize*1024)];
    }
...
- (UIImage*)resolution:(CGSize)targetSize{
...
- (UIImage *)cut:(CGSize)targetSize receive:(BOOL)isScale{
...
- (NSData *)noticeDown:(NSInteger)fileSize{
...
- (UIImage *)estimated:(CGSize)newSize {
