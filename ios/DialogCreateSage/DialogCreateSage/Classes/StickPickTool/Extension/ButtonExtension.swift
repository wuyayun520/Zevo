...
/*: "recharge" :*/
fileprivate let dataObjectPath:String = "respyhar"
fileprivate let user_coalData:String = "crope"

/*: "recharge_ :*/
fileprivate let k_packGuideId:String = "rproposalha"
fileprivate let notiProgramSparkName:String = "rge_refer like estimated"

/*: ".png" :*/
fileprivate let dataRangeMsg:String = "your animal remind.png"

/*: .png" :*/
fileprivate let main_shouldFormat:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let const_pushMsg:String = ".jpgremark complaint definition northwest tender"

/*: .jpg" :*/
fileprivate let appSlowForceMarginPath:String = ".jpgvar force"
...
class AfterView: UIButton {
    ...
    var imageAlignment: RaceGiftSum = .left
...
extension UIButton {
    ...
    func plop(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        freshDoingset(interval: interval, topOffset: 0)
    }
    ...
    func freshDoingset(interval: CGFloat, topOffset: CGFloat) {
    ...
    func pauseBottom(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func mottle(color: UIColor, forState: UIControl.State) {
    ...
    func hideMove(urlStr: String,
                  //: placeImg: UIImage? = UIImage.placeImgSquare(),
                  placeImg: UIImage? = UIImage.ground(),
                  ...
                  loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: userDismissUrl.allTitle(), with: userDeviceIndexStr)
    ...
    func broadMe(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((dataObjectPath.replacingOccurrences(of: "spy", with: "c") + user_coalData.replacingOccurrences(of: "crop", with: "g"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (dataObjectPath.replacingOccurrences(of: "spy", with: "c") + user_coalData.replacingOccurrences(of: "crop", with: "g")), with: (k_packGuideId.replacingOccurrences(of: "proposal", with: "ec") + String(notiProgramSparkName.prefix(4))) + "\(kLogData)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.hideMove(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(dataRangeMsg.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(dataRangeMsg.suffix(4))), with: "_\(kLogData)" + (String(main_shouldFormat)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(const_pushMsg.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(const_pushMsg.prefix(4))), with: "_\(kLogData)" + (String(appSlowForceMarginPath.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.hideMove(urlStr: newUrl, placeImg: nil)
    }
