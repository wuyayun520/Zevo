...
/*: "uid" :*/
fileprivate let userDigitalGenderTrueName:String = "uicost"

/*: "onlineStatus" :*/
fileprivate let appToMsg:String = "vehicleline"
fileprivate let const_bigLiveAssessmentValue:String = "answer its compute orange betterStatus"

/*: "isNewUser" :*/
fileprivate let app_ableMsg:String = "isNewUsermanager mine"

/*: "isTPAuth" :*/
fileprivate let kDrawInsertData:[Character] = ["i","s","T"]
fileprivate let user_intoStageValue:[Character] = ["P","A","u","t","h"]

/*: "totalIntimate" :*/
fileprivate let showThereMessage:[Character] = ["t","o","t","a","l","I","n","t","i","m","a","t"]
fileprivate let mainSwimTitle:[Character] = ["e"]

/*: "userStatus" :*/
fileprivate let showAllKey:[Character] = ["u","s","e","r","S","t","a","t","u","s"]

/*: "intimate" :*/
fileprivate let k_trainRelievePrincipalText:String = "intiregularate"
...
class ColumnListManager {
    ...
    private var moreMsgModel: ModelReactiveCompatible = //: return ModelReactiveCompatible.getPlaceholderModel(chatType: .moreMsg)
        .squareCap(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func magnitudeoReadBig() {
        //: self.officialArr = LightConversationListener.shared.getOfficialList()
        self.officialArr = LightConversationListener.shared.observerOnicial()
        //: self.topArr = LightConversationListener.shared.topConvList
        self.topArr = LightConversationListener.shared.topConvList
        //: self.normalArr = LightConversationListener.shared.norConvList
        self.normalArr = LightConversationListener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        transformLocation()
    ...
    func niobiteCompletion(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: LightConversationListener.shared.getChatRoomData { succeed in
        LightConversationListener.shared.hang { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = LightConversationListener.shared.chatRoomList
                self.chatRoomArr = LightConversationListener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func ok(indexPath: IndexPath) -> ModelReactiveCompatible? {
    ...
    func userModel(model: ModelReactiveCompatible?) -> ModelReactiveCompatible? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: ChinMeasurable? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: ChinMeasurable? = OverAgainInfoManager.chart(targetId: targetId)
    ...
    func per(conversationList: [ModelReactiveCompatible], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        OverAgainInfoManager.infoSelected(with: tempArr) { _, error in
    ...
    func stochasticProcess(conversationModel: ModelReactiveCompatible) {
    ...
    func currentContact(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: LightConversationListener.shared.func__getConversationList { arr, leftMore in
        LightConversationListener.shared.delay { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [ModelReactiveCompatible] = //: return Array<ModelReactiveCompatible>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [ModelReactiveCompatible] = //: return Array<ModelReactiveCompatible>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [ModelReactiveCompatible] = //: return Array<ModelReactiveCompatible>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [ModelReactiveCompatible] = //: return Array<ModelReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [ModelReactiveCompatible] = //: return Array<ModelReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [ModelReactiveCompatible] = //: return Array<ModelReactiveCompatible>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [ModelReactiveCompatible] = //: return Array<ModelReactiveCompatible>()
        .init()
    //: }()
}
...
extension ColumnListManager {
    ...
    func transformLocation() {
        //: self.moreMsgArr = LightConversationListener.shared.moreMsgList
        self.moreMsgArr = LightConversationListener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func limitRoute(indexPath: IndexPath) -> ModelReactiveCompatible? {
    ...
    func called() -> ModelReactiveCompatible {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        transformLocation()
        ...
        moreMsgModel.unreadCount = unred()
...
extension ColumnListManager {
    ...
    func ex(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        OverAgainInfoManager.qualityAge(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(userDigitalGenderTrueName.replacingOccurrences(of: "cost", with: "d"))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(appToMsg.replacingOccurrences(of: "vehicle", with: "on") + String(const_bigLiveAssessmentValue.suffix(6)))].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(app_ableMsg.prefix(9)))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(kDrawInsertData) + String(user_intoStageValue))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(showThereMessage) + String(mainSwimTitle))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(showAllKey))].int ?? 1
                    ...
                    self.sampleOld(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func betweenCombine(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(userDigitalGenderTrueName.replacingOccurrences(of: "cost", with: "d"))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(appToMsg.replacingOccurrences(of: "vehicle", with: "on") + String(const_bigLiveAssessmentValue.suffix(6)))] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(app_ableMsg.prefix(9)))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(showAllKey))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(showAllKey))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension ColumnListManager {
    ...
    func validAccountingData() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in LightConversationListener.shared.norConvList {
        for model in LightConversationListener.shared.norConvList {
            //: if model.totalIntimate >= CompositionReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= CompositionReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.userModel(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in LightConversationListener.shared.topConvList {
        for model in LightConversationListener.shared.topConvList {
            //: if model.totalIntimate >= CompositionReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= CompositionReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func negativeFeedback(convModel: ModelReactiveCompatible, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func zonePath(indexPath: IndexPath) -> ModelReactiveCompatible? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func sampleOld(uid: String, intimate: Int) {
        ...
        OverAgainInfoManager.appearance(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func train(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(userDigitalGenderTrueName.replacingOccurrences(of: "cost", with: "d"))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(k_trainRelievePrincipalText.replacingOccurrences(of: "regular", with: "m"))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
