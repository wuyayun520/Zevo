...
/*: "gifts" :*/
fileprivate let user_publicationKey:String = "giftinstruction"

/*: "vipGifts" :*/
fileprivate let kCuteKey:String = "voptionp"
fileprivate let mainDiscourseMsg:String = "smart dark existing take typeGifts"

/*: "packGifts" :*/
fileprivate let const_combineExtendFormat:String = "perform disablepackGifts"

/*: "giftSendItem" :*/
fileprivate let constAwayMessage:[Character] = ["g","i","f","t","S","e","n","d","I","t","e","m"]
...
class PoliceStrategyThen: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [YellowTitleConvertible: [Any]]]()
    // 数据锁
    ...
    private var giftType = GiftClusterLiteral.normal

    //: static let share = TalkingChatGiftManager()
    static let share = PoliceStrategyThen()
...
extension PoliceStrategyThen {
    ...
    func popularStageCompletion(type: GiftClusterLiteral = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        PoliceStrategyThen.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[YellowTitleConvertible.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            PoliceStrategyThen.share.thinProfit(completion: { isSucceed in
            ...
            PoliceStrategyThen.share.routeSucceed(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            PoliceStrategyThen.share.airBlack(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func airBlack(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = PoliceStrategyThen.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        ReminderRequestTool.capacityWho(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [YellowTitleConvertible: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(user_publicationKey.replacingOccurrences(of: "instruction", with: "s"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<SuccessHandyJSON>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[YellowTitleConvertible.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(kCuteKey.replacingOccurrences(of: "option", with: "i") + String(mainDiscourseMsg.suffix(5)))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<SuccessHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[YellowTitleConvertible.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(const_combineExtendFormat.suffix(9)))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<SuccessHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[YellowTitleConvertible.Bags] = bagsArr
                }
                ...
                var firstModel = SpeakMeasurable()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(constAwayMessage))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<SpeakMeasurable>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func thinProfit(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = PoliceStrategyThen.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        ReminderRequestTool.too(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func mainFill(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        ReminderRequestTool.belowMemory(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<WealthyMemberModel>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func routeSucceed(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = GiftClusterLiteral.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        ReminderRequestTool.minimumAgain(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func wealthyPersonPage(perPageCount: Int, dataType: YellowTitleConvertible) -> Int {
    ...
    func liftOut(dataType: YellowTitleConvertible) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func characterSetArr(dataType: YellowTitleConvertible, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension PoliceStrategyThen {
    ...
    func giveEar(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        PoliceStrategyThen.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
