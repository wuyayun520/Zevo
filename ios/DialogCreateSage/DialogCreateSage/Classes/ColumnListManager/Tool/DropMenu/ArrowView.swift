...
fileprivate let k_flagText:[UInt8] = [0x4c,0x51,0x4c,0x57,0xb,0x46,0x52,0x47,0x48,0x55,0x1d,0xc,0x3,0x4b,0x44,0x56,0x3,0x51,0x52,0x57,0x3,0x45,0x48,0x48,0x51,0x3,0x4c,0x50,0x53,0x4f,0x48,0x50,0x48,0x51,0x57,0x48,0x47]

fileprivate func titleLoad(be num: UInt8) -> UInt8 {
...
/*: "SHBDropMenuCell" :*/
fileprivate let k_damMessage:[Character] = ["S","H","B","D","r","o","p","M","e","n","u","C","e"]
fileprivate let k_markData:String = "immediateimmediate"

/*: "title" :*/
fileprivate let main_groupEnterData:String = "TITLE"
...
public class ArrowView: UIView {
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.SiblingConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.RoundComposition.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.RoundComposition.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.RoundComposition.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? DumpMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_flagText.map{titleLoad(be: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func botAlong() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func enableOp() -> UITableView {
    ...
    private func phenomenonDisappear() {
        //: addSubview(contentView)
        addSubview(contentView)
        //: mTable.register(DropMenuCell.self, forCellReuseIdentifier: "SHBDropMenuCell")
        mTable.register(MidView.self, forCellReuseIdentifier: (String(k_damMessage) + k_markData.replacingOccurrences(of: "immediate", with: "l")))
        //: contentView.addSubview(mTable)
        contentView.addSubview(mTable)
    }

    //: private func refreshUI() {
    private func chiaroscuro() {
        //: drawmTableFrame()
        exercise()
        //: mTable.reloadData()
        mTable.reloadData()
...
extension ArrowView: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(k_damMessage) + k_markData.replacingOccurrences(of: "immediate", with: "l")), for: indexPath) as! MidView
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (main_groupEnterData.lowercased()))
        //: animateRemoveView()
        anYear()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.noDelete(index: indexPath.row, title: cellModel.title ?? "")
    }
}
