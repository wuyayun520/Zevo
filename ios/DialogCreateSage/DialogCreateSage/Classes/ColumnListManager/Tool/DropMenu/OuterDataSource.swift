...
fileprivate let userDayStr:[UInt8] = [0xd6,0xd1,0xd6,0xcb,0x97,0xdc,0xd0,0xdb,0xda,0xcd,0x85,0x96,0x9f,0xd7,0xde,0xcc,0x9f,0xd1,0xd0,0xcb,0x9f,0xdd,0xda,0xda,0xd1,0x9f,0xd6,0xd2,0xcf,0xd3,0xda,0xd2,0xda,0xd1,0xcb,0xda,0xdb]

private func broadcastAdmin(spec num: UInt8) -> UInt8 {
    return num ^ 191
}

/*: "MovieView" :*/
fileprivate let appHimText:String = "DropMeresponse coat line lip wide"
fileprivate let user_exampleValue:String = "MenuCellcrop style color"

/*: "roomId" :*/
fileprivate let main_conceptKey:String = "write baseball shouldroomId"

/*: "type" :*/
fileprivate let constMoreMsg:String = "pullpe"

/*: "page" :*/
fileprivate let userLivingKey:[Character] = ["p","a","g","e"]

/*: "icon_chats_pcr" :*/
fileprivate let appSpeakFormat:String = "argumentcon"
fileprivate let k_devilPath:String = "_pcrbring float go yourself"

/*: "All Numbers" :*/
fileprivate let k_interestedData:String = "All meeting nonetheless gold dog"

/*: "all" :*/
fileprivate let notiGestureStr:[Character] = ["a","l","l"]

/*: "Recent Gifts" :*/
fileprivate let dataFingerTitle:[Character] = ["R","e","c"]
fileprivate let kLengthText:[Character] = ["e","n","t"," ","G","i","f","t","s"]
...
class OuterDataSource: UIView {
    ...
    public var triangleHeight = Config.SiblingConfig.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.SiblingConfig.adjustPullDown
    ...
    fileprivate lazy var contentView: UIView = self.resettlement()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.rest()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = OuterDataSource(frame: .zero)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userDayStr.map{broadcastAdmin(spec: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func resettlement() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func clubSpring() {
        ...
        mTable.register(MovieView.self, forCellReuseIdentifier: (String(appHimText.prefix(6)) + "mber" + String(user_exampleValue.prefix(8))))
    ...
    func frontDestroy() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(main_conceptKey.suffix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(constMoreMsg.replacingOccurrences(of: "pull", with: "ty"))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(String(userLivingKey))] = String(pageIndex)
        ...
        PoliceStrategyThen.share.mainFill(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = WealthyMemberModel()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (appSpeakFormat.replacingOccurrences(of: "argument", with: "i") + "_chats" + String(k_devilPath.prefix(4)))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(k_interestedData.prefix(4)) + "Numbers").localized
                //: model.uid = "all"
                model.uid = (String(notiGestureStr))
...
extension OuterDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(appHimText.prefix(6)) + "mber" + String(user_exampleValue.prefix(8))), for: indexPath) as! MovieView
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! WealthyMemberModel
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! WealthyMemberModel
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.pendingPrevious(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.inside(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: user_needValue, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.wateringPot()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .processAcross(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .pointOfTotalersection()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(dataFingerTitle) + String(kLengthText)).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(k_interestedData.prefix(4)) + "Numbers").localized
        }
