...
fileprivate let appHideUrl:String = "设置\u{666e}\u{901a}"
fileprivate let showVerticalUrl:String = "字颜色时 请"
fileprivate let userKingFormat:[Character] = ["使","用","R","G","B"]
fileprivate let constMeetEffectMsg:String = "\u{7a7a}间的颜色值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let show_sayPhasePath:String = "设置选中"
fileprivate let main_diskUrl:String = "字颜\u{8272}时"
fileprivate let const_hourUrl:String = " \u{8bf7}使\u{7528}RG"

/*: "init(coder:) has not been implemented" :*/
fileprivate let user_spotBringNameureFormat:[UInt8] = [0x23,0x24,0x23,0x3e,0x62,0x29,0x25,0x2e,0x2f,0x38,0x70,0x63,0x6a,0x22,0x2b,0x39,0x6a,0x24,0x25,0x3e,0x6a,0x28,0x2f,0x2f,0x24,0x6a,0x23,0x27,0x3a,0x26,0x2f,0x27,0x2f,0x24,0x3e,0x2f,0x2e]

private func quantityLabor(increase num: UInt8) -> UInt8 {
    return num ^ 74
}

/*: "设置的下标不合法!!" :*/
fileprivate let mainLateFormat:String = "设置的下标不\u{5408}法"
fileprivate let data_introMsg:[Character] = ["!","!"]

/*: "Bags" :*/
fileprivate let constGoingStr:[Character] = ["B","a","g","s"]
...
open class AuthorThen: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: ListenSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.comeUpon(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((appHideUrl + "状\u{6001}\u{7684}文" + showVerticalUrl.capitalized + String(userKingFormat) + constMeetEffectMsg))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.comeUpon(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((show_sayPhasePath.capitalized + "状态的文" + main_diskUrl + const_hourUrl + "B空间\u{7684}颜色值"))
        }
    ...
    fileprivate func comeUpon(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: ListenSegmentStyle, titles: [String]) {
    ...
    @objc func asBuild(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? SeeRecoverCustomLabel else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_spotBringNameureFormat.map{quantityLabor(increase: $0)}, encoding: .utf8)!)
    }
}
...
public extension AuthorThen {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func directionAnimated(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (mainLateFormat + String(data_introMsg)))
        ...
        localAnimate(animated)
    }

    // 暴露给外界来刷新标题的显示
    //: public func reloadTitlesWithNewTitles(_ titles: [String]) {
    func restoreFeature(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        directionAnimated(0, animated: true)
    }
}
...
//: extension ScrollSegmentView {
extension AuthorThen {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func greetSecond() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = SeeRecoverCustomLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.asBuild(_:)))
            ...
            if title == (String(constGoingStr)).localized {
    ...
    fileprivate func fill() {
        ...
        if let firstLabel = labelsArray[0] as? SeeRecoverCustomLabel {
    ...
    public func translationBegin(_ currentIndex: Int) {
...
open class SeeRecoverCustomLabel: UILabel {
    /// 用来记录当前label的缩放比例
