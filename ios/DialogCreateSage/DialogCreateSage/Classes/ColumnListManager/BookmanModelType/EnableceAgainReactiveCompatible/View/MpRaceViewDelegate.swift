...
fileprivate let k_prepareMiniFeedbackText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Online Users" :*/
fileprivate let data_localFormat:String = "animal now pop length withOnlin"
...
class MpRaceViewDelegate: UIView {
    //: private let contentV_H = (344+kDeviceSafeBottomHeight)
    private let contentV_H = (344 + userAcceptFormat)
    //: private var DataSource: [TalkingLiveRoomOnlineListModel] = []
    private var DataSource: [BalanceHandyJSON] = []
    ...
    override init(frame: CGRect) {
        ...
        setupResumeSwaddlingClothes()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: k_prepareMiniFeedbackText.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(discount), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var contentView: UIView = {
        ...
        v.produceRadii(width: user_needValue,
                       //: height: contentV_H,
                       height: contentV_H,
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.font = UIFont.counteraction(fontSize: 20)
        //: lb.textColor = UIColor.appTitleColor()
        lb.textColor = UIColor.antiquityColor()
        //: lb.textAlignment = .center
        lb.textAlignment = .center
        //: lb.text = "Online Users".localized
        lb.text = (String(data_localFormat.suffix(5)) + "e Users").localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(TwoViewCell.self, forCellReuseIdentifier: TwoViewCell.className())
...
extension MpRaceViewDelegate {
    //: private func reqData() {
    private func untilToday() {
        //: TalkingAudienceManager().req_liveRoomOnlineList(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
        TensionReactiveCompatible().oner(chatGroupId: self.chatGroupId) { succeed, result, _ in
            ...
            if succeed {
                ...
                var dataArr: [BalanceHandyJSON] = []

                //: if let datas = Array<TalkingLiveRoomOnlineListModel>.deserialize(from: array as? Array) {
                if let datas = Array<BalanceHandyJSON>.deserialize(from: array as? Array) {
                    //: dataArr.append(contentsOf: (datas as? [TalkingLiveRoomOnlineListModel] ?? []))
                    dataArr.append(contentsOf: (datas as? [BalanceHandyJSON] ?? []))
                }
...
extension MpRaceViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = TwoViewCell.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingLiveRoomOnlineListCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TwoViewCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingLiveRoomOnlineListCell(style: .default, reuseIdentifier: identifier)
            cell = TwoViewCell(style: .default, reuseIdentifier: identifier)
        }
        //: cell?.setCell(model: self.DataSource[indexPath.row], index: indexPath.row)
        cell?.translate(model: self.DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension MpRaceViewDelegate {
    ...
    func productLaunch(chatGroupId: String) {
...
extension MpRaceViewDelegate {
    ...
    private func setupResumeSwaddlingClothes() {
        ...
        let style = SceneEmptyStyle()
        //: emptyView = EmptyView.init(frame: CGRect(x: 0, y: -100, width: ScreenWidth, height: contentV_H+100), style: style)
        emptyView = EmptyView(frame: CGRect(x: 0, y: -100, width: user_needValue, height: contentV_H + 100), style: style)
