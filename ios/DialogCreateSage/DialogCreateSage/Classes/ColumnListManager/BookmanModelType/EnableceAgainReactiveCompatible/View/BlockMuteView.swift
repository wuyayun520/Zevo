...
fileprivate let appNeedKey:[UInt8] = [0x4b,0x4c,0x4b,0x56,0xa,0x41,0x4d,0x46,0x47,0x50,0x18,0xb,0x2,0x4a,0x43,0x51,0x2,0x4c,0x4d,0x56,0x2,0x40,0x47,0x47,0x4c,0x2,0x4b,0x4f,0x52,0x4e,0x47,0x4f,0x47,0x4c,0x56,0x47,0x46]

/*: "Select mute duration" :*/
fileprivate let show_adviseId:[Character] = ["S","e","l","e","c"]
fileprivate let kLineTodayData:String = "t mutresent simultaneously earn into segment"
fileprivate let userBearKey:String = "ratifrankn"

/*: "Cirform" :*/
fileprivate let const_statePath:[Character] = ["C","i","r","f","o","r"]
fileprivate let showSeekText:String = "inform"

/*: "Mute success" :*/
fileprivate let data_elementFormat:String = "Mute suaddition near quick trace"
fileprivate let show_announcementRetainRecognizeKey:[Character] = ["c","c","e","s","s"]

/*: "day" :*/
fileprivate let noti_familyStr:String = "DAY"

/*: "days" :*/
fileprivate let kBounceMessage:String = "dfollows"
...
class BlockMuteView: UIView {
    ...
    var viewType: AuthorViewType = .live
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appNeedKey.map{$0^34}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(noClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(show_adviseId) + String(kLineTodayData.prefix(5)) + "e du" + userBearKey.replacingOccurrences(of: "frank", with: "o")).localized
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
        //: return lb
        return lb
    ...
    lazy var commitBtn: UIButton = {
        ...
        btn.setTitle((String(const_statePath) + showSeekText.replacingOccurrences(of: "inform", with: "m")).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(weeklyChange), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension BlockMuteView {
    /// 消除视图按钮
    //: @objc private func bgDismissButtonClick() {
    @objc private func noClick() {
        //: dismiss()
        suspend()
    }
    ...
    func land() {
        ...
        self.contentView.frame = CGRect(x: (user_needValue - contentV_Size.width) / 2, y: kStreamTitle, width: contentV_Size.width, height: contentV_Size.height)

        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = (ScreenHeight-self.contentV_Size.height)/2
            self.contentView.frame.origin.y = (kStreamTitle - self.contentV_Size.height) / 2
        }
    }
    ...
    func suspend() {
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.contentView.frame.origin.y = kStreamTitle
...
extension BlockMuteView {
    ...
    @objc private func halfBtn(btn: UIButton) {
    ...
    @objc private func weeklyChange() {
        ...
        if self.viewType == .live {
            ...
            TensionReactiveCompatible().thenObject(targetUid: targetUid, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                PointerReactiveCompatible.motivationCharmDismiss()
                //: if succeed {
                if succeed {
                    //: self.func__showStatusBarSuccessMsg(showMsg: "Mute success".localized)
                    self.through(showMsg: (String(data_elementFormat.prefix(7)) + String(show_announcementRetainRecognizeKey)).localized)
                }
                //: self.dismiss()
                self.suspend()
            }
...
extension BlockMuteView {
    ...
    private func exceptExtended() {
        ...
        for i in 0 ..< 3 {
            ...
            btn.mottle(color: .white, forState: .normal)
            //: btn.setBackgroundColor(color: UIColor.appThemeColor(), forState: .selected)
            btn.mottle(color: UIColor.rawMonthCombine(), forState: .selected)
            ...
            let dayStr = (title == "1") ? (noti_familyStr.lowercased()).localized : (kBounceMessage.replacingOccurrences(of: "follow", with: "ay")).localized
            //: btn.setTitle("\(title) " + dayStr, for: .normal)
            btn.setTitle("\(title) " + dayStr, for: .normal)
            //: btn.tag = 100+i
            btn.tag = 100 + i
            //: btn.addTarget(self, action: #selector(muteBtnClick), for: .touchUpInside)
            btn.addTarget(self, action: #selector(halfBtn), for: .touchUpInside)
