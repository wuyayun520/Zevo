...
fileprivate let show_flagContent:[UInt8] = [0xbb,0xbc,0xbb,0xa6,0xfa,0xb1,0xbd,0xb6,0xb7,0xa0,0xe8,0xfb,0xf2,0xba,0xb3,0xa1,0xf2,0xbc,0xbd,0xa6,0xf2,0xb0,0xb7,0xb7,0xbc,0xf2,0xbb,0xbf,0xa2,0xbe,0xb7,0xbf,0xb7,0xbc,0xa6,0xb7,0xb6]
...
enum PromiseAdditiveArithmetic: Int {
    //: case TimeGap = 0
...
@objc protocol ArrowViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func getAlongAlbum()

    //: @objc optional func endScroll()
    @objc optional func deuteranopic()
}

//: class TalkingDanmuMsgListTableView: UIView {
class SiblingHelpViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_flagContent.map{$0^210}, encoding: .utf8)!)
    }
...
extension SiblingHelpViewDelegate {
    //: func startTimer() {
    func scraping() {
        //: stopTimer()
        pullUp()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(pingPure), userInfo: nil, repeats: true)
    }
...
extension SiblingHelpViewDelegate {
    ...
    func teaser(msgModel: MedalHandyJSON?) {
    ...
    func backUp() {
        ...
        if AllHeight > show_meetingValue {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < show_meetingValue {
                ...
                self.msgTableView.height = show_meetingValue
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = show_meetingValue - AllHeight
        ...
        happeningAnimated(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func happeningAnimated(animated: Bool) {
    ...
    func oeuvreFeature(inPending: Bool) {
...
extension SiblingHelpViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func beforeTipWith(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.deuteranopic?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(showVideoTableStr)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = StrategyBassCell.deviate(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension SiblingHelpViewDelegate {
    ...
    func setupWrapSwaddlingClothes() {
