...
fileprivate let k_fadeUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
enum VisualCommunicationSightTypePropertyProtocol: Int {
    //: case TimeGap = 0
...
@objc protocol ReminderViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func cater()

    //: @objc optional func endScroll()
    @objc optional func ridScroll()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class PreviouslyTableView: UIView {
    ...
    var msgArray = [DanmuHandyJSON]() /// 消息数组(数据源)
    //: var  tempMsgArray = Array<TalkingVideoCallDanmuModel>()
    var tempMsgArray = [DanmuHandyJSON]() /// 用于存储消息还未刷新到tableView的时候接收到的消息
    //: var inPending = false
    var inPending = false /// 是否处于爬楼状态
    //: var refreshTimer: Timer?
    var refreshTimer: Timer? /// 刷新定时器
    //: var reloadType: VideoReloadLiveMsgRoomType?
    var reloadType: VisualCommunicationSightTypePropertyProtocol?
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(diskShrink))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_fadeUrl.reversed(), encoding: .utf8)!)
    }
...
extension PreviouslyTableView {
    ...
    func audacious() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(color), userInfo: nil, repeats: true)
    }
    ...
    @objc func color() {
        ...
        sessionSegmentBottom()
    }

    //: @objc func tapAction() {
    @objc func diskShrink() {
...
extension PreviouslyTableView {
    ...
    func addNeedModel(msgModel: DanmuHandyJSON?) {
    ...
    func realiseBottom() {
        ...
        if AllHeight > user_indexMsg {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < user_indexMsg {
                ...
                self.msgTableView.height = user_indexMsg
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = user_indexMsg - AllHeight
        ...
        busyLevel(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func busyLevel(animated: Bool) {
...
extension PreviouslyTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func way(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.ridScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(noti_netMessage)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = RaceReactiveCompatible.frank(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension PreviouslyTableView {
    ...
    func subviewsEndFile() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: user_gamePath, height: user_indexMsg)
    }
}
