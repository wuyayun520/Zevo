...
fileprivate let showRecoverKey:[UInt8] = [0x58,0x5f,0x58,0x45,0x19,0x52,0x5e,0x55,0x54,0x43,0xb,0x18,0x11,0x59,0x50,0x42,0x11,0x5f,0x5e,0x45,0x11,0x53,0x54,0x54,0x5f,0x11,0x58,0x5c,0x41,0x5d,0x54,0x5c,0x54,0x5f,0x45,0x54,0x55]

private func stripRegular(spot num: UInt8) -> UInt8 {
    return num ^ 49
}
...
class CastView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showRecoverKey.map{stripRegular(spot: $0)}, encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        FileSocketManager.shared.containerView?.isHidden = false
    }
    ...
    func saucer() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= user_needValue * 0.5) ? -self.width : user_needValue + self.width
            ...
            FileSocketManager.shared.containerView?.isHidden = true
        }
    }
}
...
extension CastView {
    ...
    func addDoing() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.fallFor(sender:)))
    ...
    func archaic() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.panelSender(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.addDoing()
    ...
    @objc func panelSender(sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .ended {
            ...
            FileSocketManager.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func fallFor(sender _: UIPanGestureRecognizer) {
