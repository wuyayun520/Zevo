...
fileprivate let noti_edibleFormat:[UInt8] = [0x7e,0x79,0x7e,0x63,0x3f,0x74,0x78,0x73,0x72,0x65,0x2d,0x3e,0x37,0x7f,0x76,0x64,0x37,0x79,0x78,0x63,0x37,0x75,0x72,0x72,0x79,0x37,0x7e,0x7a,0x67,0x7b,0x72,0x7a,0x72,0x79,0x63,0x72,0x73]

private func printingUnit(seat num: UInt8) -> UInt8 {
    return num ^ 23
}

/*: "#FFD13A" :*/
fileprivate let notiErrorRestrictionData:[Character] = ["#","F","F","D","1","3","A"]

/*: "gift_gb_umnad_1" :*/
fileprivate let notiObjectShirtTitle:[Character] = ["g","i","f","t","_","g","b","_","u","m","n","a","d","_","1"]

/*: "icon_circle_pic_default" :*/
fileprivate let userListStr:String = "icok"
fileprivate let data_outcomeMsg:String = "give price doc bug find_circl"
fileprivate let noti_wellName:String = "_defaulttun premium appropriate maker north"

/*: "Open lucky blind-box" :*/
fileprivate let main_blanketId:[Character] = ["O","p","e","n"," ","l","u"]
fileprivate let constLineTingPreserveTitle:[Character] = ["c","k","y"," ","b","l","i","n","d","-","b","o","x"]

/*: "Sent " :*/
fileprivate let dataIncomeApplyStr:String = "dynamics type unable medal textSent "

/*: "PingFangSC-Semibold" :*/
fileprivate let main_retirementMsg:[Character] = ["P","i","n","g","F","a","n","g","S"]
fileprivate let show_statusName:[Character] = ["C","-","S","e","m","i","b","o","l","d"]

/*: "Bombard" :*/
fileprivate let notiSplitName:String = "point remarkBombard"
...
class LightReactiveCompatible: UIView {
    ...
    var giftMsgModel = RecoverAnimatModel()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [SpeakMeasurable()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_edibleFormat.map{printingUnit(seat: $0)}, encoding: .utf8)!)
    }
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(notiErrorRestrictionData)))
        //: lb.font = UIFont.pingfangRugularFont(fontSize: 13)
        lb.font = UIFont.joinSize(fontSize: 13)
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var pluralTip: SortBenchThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = SortBenchThen()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: SortBenchThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = SortBenchThen()
...
extension LightReactiveCompatible {
    ...
    func duringAcquisitionModel(model: RecoverAnimatModel) {
        ...
        self.bgImage.image = UIImage.parentSet(name: (String(notiObjectShirtTitle)))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.parentSet(name: (userListStr.replacingOccurrences(of: "ok", with: "on") + String(data_outcomeMsg.suffix(6)) + "e_pic" + String(noti_wellName.prefix(8))))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == MaximumAnimatType.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(main_blanketId) + String(constLineTingPreserveTitle)).localized
            //: }else {
        } else {
        ...
        self.avatarImage.funReply(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.funReply(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(dataIncomeApplyStr.suffix(5))).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(main_retirementMsg) + String(show_statusName)), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = beyondAlbum(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(main_retirementMsg) + String(show_statusName)), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = beyondAlbum(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.tab(texts: strs, colors: colors, fonts: fonts)
        ...
        theChoose(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func theChoose(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(main_retirementMsg) + String(show_statusName)), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = beyondAlbum(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(main_retirementMsg) + String(show_statusName)), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = beyondAlbum(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.pushDown(150, 77, 20, 1), UIColor.white, UIColor.pushDown(150, 77, 20, 1)]
        ...
        attText = attText.tab(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.exitApplication(attributedText: attText)
        ...
        countLb.tickLayOver(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func separate(giftMsgModel: RecoverAnimatModel) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(executeHide), object: nil)
        ...
        self.perform(#selector(executeHide), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func liveAnimate(model _: RecoverAnimatModel, animate _: Bool) {
    ...
    @objc func executeHide() {
        //: NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(animateHide), object: nil)
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(executeHide), object: nil)
        //: giftMsgModel = TalkingGiftMsgAnimatModel()
        giftMsgModel = RecoverAnimatModel()
...
extension LightReactiveCompatible {
    ...
    func beyondAlbum(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension LightReactiveCompatible {
    ...
    private func subviewsGuide() {
        ...
        font = beyondAlbum(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.tab(texts: [(String(notiSplitName.suffix(7))).localized], colors: [UIColor.pushDown(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.tickLayOver(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.exitApplication(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.tickLayOver(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
