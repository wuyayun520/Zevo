...
/*: "call_bgm" :*/
fileprivate let main_botId:[Character] = ["c"]
fileprivate let data_resourceEarlyMessage:String = "all_bgmyes period mutual human"

/*: "call_response_bgm" :*/
fileprivate let mainActiveReceiveUrl:[Character] = ["c","a","l","l","_","r","e","s","p"]
fileprivate let show_restrictionStr:String = "ON"
fileprivate let data_resourceId:String = "se_bgmeff family sit inner tolerance"

/*: "invitePop_call_bgm" :*/
fileprivate let noti_noticeMsg:String = "allvite"
fileprivate let noti_reminderData:String = "all_bbrow narrow white"
fileprivate let appNobodyId:[Character] = ["g","m"]
...
enum SightDefaultsSerializable: Int {
    //: case Call
    case Call // 音视频拨打音乐
    //: case Response
...
class ExecCosPlayer: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = SightDefaultsSerializable.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func layerType(type: SightDefaultsSerializable) -> ExecCosPlayer {
        //: let player = TalkingCallBGMPlayer()
        let player = ExecCosPlayer()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.unify()
...
extension ExecCosPlayer {
    ...
    private func latissimusDorsiScreen() {
    ...
    private func counterrupt(notify: Notification) {
    ...
    private func nameNotify(notify: Notification) {
    ...
    private func organicStructureAsk() -> Bool {
    ...
    private func unify() {
        ...
        var fileName = (String(main_botId) + String(data_resourceEarlyMessage.prefix(7)))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(mainActiveReceiveUrl) + show_restrictionStr.lowercased() + String(data_resourceId.prefix(6)))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (noti_noticeMsg.replacingOccurrences(of: "all", with: "in") + "Pop_c" + String(noti_reminderData.prefix(5)) + String(appNobodyId))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = AuthorReactiveCompatible.default.guideType(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension ExecCosPlayer: TXVodPlayListener {
