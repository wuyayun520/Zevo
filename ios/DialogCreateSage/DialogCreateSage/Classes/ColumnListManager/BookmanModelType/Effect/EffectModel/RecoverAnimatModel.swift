...
/*: "num" :*/
fileprivate let dataLayUrl:[Character] = ["n","u","m"]
...
@objc public enum MaximumAnimatType: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct RecoverAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// MaximumAnimatType
    ...
    mutating func createer(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct AppearanceModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(dataLayUrl))
    }
}
