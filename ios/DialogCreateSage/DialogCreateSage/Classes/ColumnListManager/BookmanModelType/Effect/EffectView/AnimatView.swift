...
fileprivate let dataCeremonyText:[UInt8] = [0xc7,0xc0,0xc7,0xda,0x86,0xcd,0xc1,0xca,0xcb,0xdc,0x94,0x87,0x8e,0xc6,0xcf,0xdd,0x8e,0xc0,0xc1,0xda,0x8e,0xcc,0xcb,0xcb,0xc0,0x8e,0xc7,0xc3,0xde,0xc2,0xcb,0xc3,0xcb,0xc0,0xda,0xcb,0xca]

private func wordTone(editor num: UInt8) -> UInt8 {
    return num ^ 174
}

/*: "iosSvgaEffect" :*/
fileprivate let noti_totalValue:[Character] = ["i","o","s","S","v"]
fileprivate let user_burnBucketPath:String = "gresume"
fileprivate let mainTextValue:[Character] = ["E","f","f","e","c","t"]

/*: "iosEffect" :*/
fileprivate let const_entireStr:String = "iosEfnegative exact pic age"
fileprivate let noti_regionValue:String = "fenightt"

/*: "entityId" :*/
fileprivate let notiRedName:String = "ting fort rewardentityI"
fileprivate let noti_remoteId:String = "opening"
...
class AnimatView: UIView {
    ...
    var currentEffectMsgModel: BookmanModelType?
    ...
    var animatedView: FinishReactiveCompatible?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: HelpCompositionStepProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataCeremonyText.map{wordTone(editor: $0)}, encoding: .utf8)!)
    }
...
extension AnimatView {
    ...
    func activityDic(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(noti_totalValue) + user_burnBucketPath.replacingOccurrences(of: "resume", with: "a") + String(mainTextValue))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(const_entireStr.prefix(5)) + noti_regionValue.replacingOccurrences(of: "night", with: "c"))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(notiRedName.suffix(7)) + noti_remoteId.replacingOccurrences(of: "opening", with: "d"))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = haveAdmin(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func haveAdmin(msgModel: Any?) -> Bool {
        ...
        if msgModel is BookmanModelType {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? BookmanModelType
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is RecoverAnimatModel {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! RecoverAnimatModel
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.createer(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = BookmanModelType()
            ...
            effectMsgModel = BookmanModelType()
            ...
            if giftMsgModel.showType == MaximumAnimatType.myStery.rawValue || giftMsgModel.showType == MaximumAnimatType.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: BookmanModelType = self.effectMsgModelArray[i] as! BookmanModelType
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? BookmanModelType())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? BookmanModelType(), at: index)
                }
            ...
            oatmealCookieAnswer(effectMsgModel: effectMsgModel!)
        }
...
extension AnimatView {
    ...
    func debate() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = checkion() else { return }
                //: firstShow(effectMsgModel: removeModel)
                oatmealCookieAnswer(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.limit()
        }
    ...
    func checkion() -> BookmanModelType? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return BookmanModelType()
        }
        ...
        if effectMsgModel is BookmanModelType {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! BookmanModelType
        ...
        return effectMsgModel as? BookmanModelType
    }
}
...
extension AnimatView {
    ...
    func oatmealCookieAnswer(effectMsgModel: BookmanModelType) {
        ...
        self.delegate?.attributeObj(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        StrategyMovieThen.shared.serviceCompletion(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.intervaleractionExist(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? InformationMeasurable())
            //: })
        })
    }
    ...
    func intervaleractionExist(effectMsgModel: BookmanModelType, success: Bool, effectModel: InformationMeasurable) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.unlessDot(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    identity(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func coverModel(effectMsgModel: BookmanModelType, effectModel: InformationMeasurable) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.sinceNameure()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.factorFinishAnimating()
        ...
        className = StrategyMovieThen.shared.dismissTap(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! FinishReactiveCompatible.Type
...
extension AnimatView: AidReactiveCompatible {
    ...
    func now(effectItemView: FinishReactiveCompatible, success: Bool) {
        ...
        if !success {
            ...
            representation(success: success)
        }
        ...
        self.delegate?.situation(effectView: self, obj: effectItemView.effectMsgModel ?? BookmanModelType(), success: success)
    }
    ...
    func roundSign(effectItemView: FinishReactiveCompatible) {
        ...
        self.delegate?.video(effectView: self, obj: effectMsgModel!)
    }
}
...
extension AnimatView {
    ...
    // clean
    //: func cleanData() {
    func enableDirection() {
        //: if animatedView != nil {
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.sinceNameure() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.factorFinishAnimating() // clean animation data
    ...
    func representation(success: Bool) {
        //: cleanData()
        enableDirection()
