...
fileprivate let main_saleFormat:[UInt8] = [0x4c,0x4b,0x4c,0x51,0xd,0x46,0x4a,0x41,0x40,0x57,0x1f,0xc,0x5,0x4d,0x44,0x56,0x5,0x4b,0x4a,0x51,0x5,0x47,0x40,0x40,0x4b,0x5,0x4c,0x48,0x55,0x49,0x40,0x48,0x40,0x4b,0x51,0x40,0x41]

private func withAction(keep num: UInt8) -> UInt8 {
    return num ^ 37
}

/*: "icon_chat_lock" :*/
fileprivate let constSimultaneouslyContent:String = "icoyellow"
fileprivate let kTransformData:String = "t_lockalongside trust person unable color"

/*: "icon_chat_burned" :*/
fileprivate let appAdditionalUrl:String = "sit going strategy apartmenticon_"
fileprivate let k_purchaseMsg:String = "translate green partner closed remaining_burn"
fileprivate let const_curveValue:[Character] = ["e","d"]

/*: "icon_chat_burned_shan" :*/
fileprivate let kDetailedUrl:String = "icreading"
fileprivate let appProjectName:String = "_burneproduct drop offer floor"
fileprivate let showLiftFormat:String = "d_shanad con writing when"

/*: "Already burned" :*/
fileprivate let constStructureValue:String = "address followingAlread"
...
class CastMsgCell: FoundViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_saleFormat.map{withAction(keep: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.parentSet(name: (constSimultaneouslyContent.replacingOccurrences(of: "yellow", with: "n") + "_cha" + String(kTransformData.prefix(6))))
    ...
    lazy var burnedImagV: AfterView = {
        ...
        imgV.setBackgroundImage(UIImage.parentSet(name: (String(appAdditionalUrl.suffix(5)) + "chat" + String(k_purchaseMsg.suffix(5)) + String(const_curveValue))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.parentSet(name: (kDetailedUrl.replacingOccurrences(of: "reading", with: "on") + "_chat" + String(appProjectName.prefix(6)) + String(showLiftFormat.prefix(6)))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(constStructureValue.suffix(6)) + "y burned").localized, for: .normal)
...
extension CastMsgCell {
    //: override func fill(with data: TCommonCellData) {
    override func hitch(with data: StrainThen) {
        //: super.fill(with: data)
        super.hitch(with: data)
        //: guard let imageCellData = data as? MilitaryCellData else { return }
        guard let imageCellData = data as? MilitaryCellData else { return }
        ...
        let intimatePhoto: OnerModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            distant(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            distant(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: MilitaryCellData, urlStr: String?, isMosaic: Bool) {
    func distant(data: MilitaryCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.resign(image, smooth: 90)
                        self.photoView.image = UIImage.resign(image, smooth: 90)
                        //: } else {
                    } else {
            ...
            checkNumbero(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.agreeFinish(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func checkNumbero(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.dotRidIdentity(urlStr: urlStr) { [weak self] image in
