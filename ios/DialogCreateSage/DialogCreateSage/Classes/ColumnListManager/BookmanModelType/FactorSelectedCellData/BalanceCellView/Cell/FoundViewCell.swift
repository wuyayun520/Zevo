...
fileprivate let mainMultipleMessage:[UInt8] = [0x2d,0x2a,0x2d,0x30,0x6c,0x27,0x2b,0x20,0x21,0x36,0x7e,0x6d,0x64,0x2c,0x25,0x37,0x64,0x2a,0x2b,0x30,0x64,0x26,0x21,0x21,0x2a,0x64,0x2d,0x29,0x34,0x28,0x21,0x29,0x21,0x2a,0x30,0x21,0x20]

/*: "icon_lounge" :*/
fileprivate let app_staffData:String = "balance own approval phase priorityicon_l"

/*: "icon_talk_points" :*/
fileprivate let data_effectiveUrl:String = "icon_tfour find"
fileprivate let show_manRunningName:[Character] = ["i","n","t","s"]

/*: "C6BDFF" :*/
fileprivate let notiGivenPath:String = "CequalDFF"

/*: "#BBBBBB" :*/
fileprivate let const_informationMsg:String = "capture print twenty stack#BBBBBB"

/*: "Reply to get points~" :*/
fileprivate let const_squareName:[Character] = ["R","e","p","l","y"," ","t","o"," ","g","e"]
fileprivate let dataDefinitionUrl:[Character] = ["t"," ","p"]
fileprivate let show_floorText:[Character] = ["o","i","n","t","s","~"]

/*: "bth_unread_nor" :*/
fileprivate let userRefuseTitle:String = "bth_uconnect factor"
fileprivate let notiSoundStr:String = "agreeea"

/*: "icon_male_default" :*/
fileprivate let app_smartId:[UInt8] = [0x86,0x8c,0x80,0x81,0xb0,0x82,0x8e,0x83,0x8a,0xb0,0x8b,0x8a,0x89,0x8e,0x9a,0x83,0x9b]

/*: "icon_female_default" :*/
fileprivate let app_seeFormat:[Character] = ["i","c","o","n","_","f","e","m","a","l","e","_","d","e","f","a","u"]
fileprivate let app_boxTitle:[Character] = ["l","t"]

/*: "+%@ points" :*/
fileprivate let showArrayData:String = "+%@ podeliver staff join result opening"
fileprivate let data_prettyMessage:[Character] = ["i","n","t","s"]

/*: "bth_read_pre" :*/
fileprivate let const_schedulePath:[Character] = ["b","t","h","_","r"]
fileprivate let show_plainDrawValue:String = "successfully revenue inner maximumead_pre"
...
//: class TalkingChatBaseMsgCell: ReaderView {
class FoundViewCell: ReaderView {
    //: var cellData: TalkingChatMsgBaseCellData?
    var cellData: BlockCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainMultipleMessage.map{$0^68}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.parentSet(name: (String(app_staffData.suffix(6)) + "ounge"))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var ageLab: UILabel = {
        ...
        label.font = UIFont.counteraction(fontSize: 10)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var coinIconImg: UIImageView = {
        ...
        imgV.image = UIImage.parentSet(name: (String(data_effectiveUrl.prefix(6)) + "alk_po" + String(show_manRunningName)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    public lazy var coinLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (notiGivenPath.replacingOccurrences(of: "equal", with: "6B")))
        //: label.isHidden = true
        label.isHidden = true
    ...
    public lazy var replyTipLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(const_informationMsg.suffix(7))))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.joinSize(fontSize: 14)
        //: label.text = "Reply to get points~".localized
        label.text = (String(const_squareName) + String(dataDefinitionUrl) + String(show_floorText)).localized
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var isReadImg: UIImageView = {
        ...
        imgV.image = UIImage.parentSet(name: (String(userRefuseTitle.prefix(5)) + notiSoundStr.replacingOccurrences(of: "agree", with: "nr") + "d_nor"))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension FoundViewCell {
    ...
    override func hitch(with data: StrainThen) {
        ...
        self.cellData = data as? BlockCellData
        ...
        let userModel = newData.msgModel.user
        //: let loungePlus = (CompositionReactiveCompatible.share.loginUid == String(userModel.uid)) ? CompositionReactiveCompatible.share.loginUserMode.loungePlus : userModel.loungePlus
        let loungePlus = (CompositionReactiveCompatible.share.loginUid == String(userModel.uid)) ? CompositionReactiveCompatible.share.loginUserMode.loungePlus : userModel.loungePlus

        //: self.nameLabel.text = userModel.nickname
        self.nameLabel.text = userModel.nickname
        //: self.nameLabel.textColor = loungePlus ? .userVipColor() : .appValueColor()
        self.nameLabel.textColor = loungePlus ? .colorful() : .submaxilla()
        //: self.nameLabel.font = UIFont.pingfangMediumFont(fontSize: 13)
        self.nameLabel.font = UIFont.counteraction(fontSize: 13)
        //: let imgStr = userModel.sex == Int(Gender.male.rawValue) ? "icon_male_default" : "icon_female_default"
        let imgStr = userModel.sex == Int(ArrowGender.male.rawValue) ? String(bytes: app_smartId.map{$0^239}, encoding: .utf8)! : (String(app_seeFormat) + String(app_boxTitle))
        //: sexIconBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexIconBtn.setBackgroundImage(UIImage.parentSet(name: imgStr), for: .normal)
        ...
        self.avatarView.agreeFinish(urlStr: userModel.headPic)
        ...
        self.iconBorder.broadMe(urlStr: userModel.headPicFrame)

        //: let isShowCoin = newData.msgIncome > 0
        let isShowCoin = newData.msgIncome > 0
        //: self.coinIconImg.isHidden = !(isShowCoin && CompositionReactiveCompatible.share.appStatus == AppSkinStatus.normal.rawValue)
        self.coinIconImg.isHidden = !(isShowCoin && CompositionReactiveCompatible.share.appStatus == ColumnExInsetTarget.normal.rawValue)
        ...
        self.coinLabel.text = (String(showArrayData.prefix(6)) + String(data_prettyMessage)).temporaryWorker(NSNumber(value: Float(newData.msgIncome)))
        // 女性逻辑, 未回复付费消息提示
        //: if TalkingPrivateChatManager.needShowReplyTipsMessage(newData.direction,
        if ChockThen.speedy(newData.direction,
        ...
        if ChockThen.productCell(cellData: newData) {
            //: self.isReadImg.isHidden = false
            self.isReadImg.isHidden = false
            //: var readImgStr = "bth_unread_nor"
            var readImgStr = (String(userRefuseTitle.prefix(5)) + notiSoundStr.replacingOccurrences(of: "agree", with: "nr") + "d_nor")
            // 消息对端是否已读
            //: if TalkingPrivateChatManager.msgIsRead(cellData: newData) {
            if ChockThen.meanDistance(cellData: newData) {
                //: readImgStr = "bth_read_pre"
                readImgStr = (String(const_schedulePath) + String(show_plainDrawValue.suffix(7)))
            }
            //: self.isReadImg.image = UIImage.BundleImageNamed(name: readImgStr)
            self.isReadImg.image = UIImage.parentSet(name: readImgStr)
    ...
    override public func updateConstraints() {
        ...
        if let temCellData = self.cellData {
            //: loungePlus = (CompositionReactiveCompatible.share.loginUid == String(temCellData.msgModel.user.uid)) ? CompositionReactiveCompatible.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
            loungePlus = (CompositionReactiveCompatible.share.loginUid == String(temCellData.msgModel.user.uid)) ? CompositionReactiveCompatible.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
        }
