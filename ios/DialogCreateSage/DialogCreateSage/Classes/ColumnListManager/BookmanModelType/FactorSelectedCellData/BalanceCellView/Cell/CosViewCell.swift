...
fileprivate let app_dailyUrl:[UInt8] = [0xac,0xab,0xac,0xb1,0xed,0xa6,0xaa,0xa1,0xa0,0xb7,0xff,0xec,0xe5,0xad,0xa4,0xb6,0xe5,0xab,0xaa,0xb1,0xe5,0xa7,0xa0,0xa0,0xab,0xe5,0xac,0xa8,0xb5,0xa9,0xa0,0xa8,0xa0,0xab,0xb1,0xa0,0xa1]

/*: "all" :*/
fileprivate let kAskFutureMsg:String = "ALL"
...
class CosViewCell: FoundViewCell {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: MaximumThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_dailyUrl.map{$0^197}, encoding: .utf8)!)
    }
...
extension CosViewCell {
    ...
    override func hitch(with data: StrainThen) {
        ...
        self.giftData = data as? MaximumThen
        ...
        self.giftImgView.agreeFinish(urlStr: imgPreview)
        //: self.giftContentLab.attributedText = giftData.attributedString
        self.giftContentLab.attributedText = giftData.attributedString
        //: if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
        if giftData.msgModel.toUser.uid == (kAskFutureMsg.lowercased()) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.parentSet(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.agreeFinish(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
