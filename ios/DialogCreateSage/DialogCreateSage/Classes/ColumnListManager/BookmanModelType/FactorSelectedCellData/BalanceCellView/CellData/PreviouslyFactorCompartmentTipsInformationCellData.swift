...
fileprivate let notiTopContent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "View read receipts?" :*/
fileprivate let main_shadeId:[Character] = ["V","i","e","w"," ","r","e","a","d"," ","r","e"]
fileprivate let showCharacteristicPath:String = "propose suiteceipts?"

/*: "text" :*/
fileprivate let kBelowTopicMsg:[Character] = ["t","e","x","t"]

/*: "color" :*/
fileprivate let const_bridgeMsg:[UInt8] = [0xfe,0xf2,0xf1,0xf2,0xef]

/*: "<xml>%@</xml>" :*/
fileprivate let notiEnableicialArmValue:String = "about beneath listen create union<xml>%"
fileprivate let main_violationUrl:String = "abs quitml>"

/*: "href" :*/
fileprivate let k_backMessage:String = "hrtopf"

/*: "type" :*/
fileprivate let app_temporaryPath:[UInt8] = [0x65,0x70,0x79,0x74]

/*: "txt" :*/
fileprivate let noti_topicMsg:String = "calculationxcalculation"

/*: "padding" :*/
fileprivate let constDocumentMessage:String = "realm"
fileprivate let userAllowApplyStr:[Character] = ["a","d","d","i","n","g"]

/*: "size" :*/
fileprivate let appDisappearPath:String = "sizcognitive"

/*: "maxFont" :*/
fileprivate let kCountSaltValue:String = "full oncemaxFont"

/*: "link" :*/
fileprivate let user_laterUrl:[Character] = ["l","i","n","k"]

/*: "PingFangSC-Regular" :*/
fileprivate let kListKey:String = "lower jump reader beneath wrapPing"
fileprivate let kPreviousTitle:String = "environment bearC-Regular"

/*: "content" :*/
fileprivate let mainKingEntireName:[UInt8] = [0x74,0x6e,0x65,0x74,0x6e,0x6f,0x63]
...
// __CLOSE_PRINT__
//
//  PreviouslyFactorCompartmentTipsInformationCellData.swift
//  AbroadTalking
//
...
@objcMembers public class PreviouslyFactorCompartmentTipsInformationCellData: BlockCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiTopContent.reversed(), encoding: .utf8)!)
    }

    //: public override func heightOfWidth(_ width: CGFloat) ->CGFloat {
    override public func clubMargin(_: CGFloat) -> CGFloat {
        //: return contentSize().height+20
        return secretoryPhase().height + 20
    }
    ...
    override public func secretoryPhase() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(main_shadeId) + String(showCharacteristicPath.suffix(7))).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: user_needValue, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: DirtyMacroDefine.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: DirtyMacroDefine.expressStackWidth() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = used(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
extension PreviouslyFactorCompartmentTipsInformationCellData {
    //: func formatMessageToStringM(info: AbTalkingChatMsgTipsModel) -> NSMutableAttributedString {
    func used(info: StereoDogModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return resumeBridge(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func resumeBridge(tips: String) -> NSMutableAttributedString {
        ...
        _ = column(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: DirtyMacroDefine.getScreenWidth()-50, para: para)
        self.yyLayout = utilised(array: attrbutedArray, displayMaxWidth: DirtyMacroDefine.expressStackWidth() - 50, para: para)
    ...
    func column(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = saveAm(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(String(kBelowTopicMsg))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: const_bridgeMsg.map{$0^157}, encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: const_bridgeMsg.map{$0^157}, encoding: .utf8)!, (String(kBelowTopicMsg))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = licensePassageColor(content: text, color: textColor)
    ...
    func saveAm(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(notiEnableicialArmValue.suffix(6)) + "@</x" + String(main_violationUrl.suffix(3))), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            medal(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: const_bridgeMsg.map{$0^157}, encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(String(kBelowTopicMsg)): newSource, String(bytes: const_bridgeMsg.map{$0^157}, encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func medal(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(k_backMessage.replacingOccurrences(of: "top", with: "e"))] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(k_backMessage.replacingOccurrences(of: "top", with: "e"))] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                medal(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(String(kBelowTopicMsg))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func utilised(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = page(array: array, paragraphStyle: para)
    ...
    func page(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, _, _ in
            ...
            let type = newObj[String(bytes: app_temporaryPath.reversed(), encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (noti_topicMsg.replacingOccurrences(of: "calculation", with: "t")) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = birth(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (constDocumentMessage.replacingOccurrences(of: "realm", with: "p") + String(userAllowApplyStr)) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(appDisappearPath.replacingOccurrences(of: "cognitive", with: "e"))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(kCountSaltValue.suffix(7)))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: const_bridgeMsg.map{$0^157}, encoding: .utf8)!] as? UIColor)
    ...
    func thirdDict(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(appDisappearPath.replacingOccurrences(of: "cognitive", with: "e"))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(kCountSaltValue.suffix(7)))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func birth(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: app_temporaryPath.reversed(), encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (String(user_laterUrl)) && type != (noti_topicMsg.replacingOccurrences(of: "calculation", with: "t")) {
        ...
        let color = dict[String(bytes: const_bridgeMsg.map{$0^157}, encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(appDisappearPath.replacingOccurrences(of: "cognitive", with: "e"))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(kListKey.suffix(4)) + "FangS" + String(kPreviousTitle.suffix(9))) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: mainKingEntireName.reversed(), encoding: .utf8)!] != nil) ? dict[String(bytes: mainKingEntireName.reversed(), encoding: .utf8)!] : ""
    ...
    func on() -> NSMutableDictionary {
        //: let font = UIFont.pingfangRugularFont(fontSize: 15)
        let font = UIFont.joinSize(fontSize: 15)
    ...
    func licensePassageColor(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return someone(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: app_temporaryPath.reversed(), encoding: .utf8)!: (noti_topicMsg.replacingOccurrences(of: "calculation", with: "t")),
                //: "content": content,
                String(bytes: mainKingEntireName.reversed(), encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: const_bridgeMsg.map{$0^157}, encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func someone(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
