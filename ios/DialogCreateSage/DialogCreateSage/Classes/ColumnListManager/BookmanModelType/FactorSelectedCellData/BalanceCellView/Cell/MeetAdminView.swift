...
fileprivate let app_othersMsg:[UInt8] = [0xb6,0xb1,0xb6,0xab,0xf7,0xbc,0xb0,0xbb,0xba,0xad,0xe5,0xf6,0xff,0xb7,0xbe,0xac,0xff,0xb1,0xb0,0xab,0xff,0xbd,0xba,0xba,0xb1,0xff,0xb6,0xb2,0xaf,0xb3,0xba,0xb2,0xba,0xb1,0xab,0xba,0xbb]

private func accompanyCounteraction(beauty num: UInt8) -> UInt8 {
    return num ^ 223
}

/*: "move" :*/
fileprivate let appTotalroId:String = "MOVE"
...
class MeetAdminView: UIView {
    ...
    var danumuMsgArray: [CastMeasurable] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [HearReactiveCompatible] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<CastMeasurable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_othersMsg.map{accompanyCounteraction(beauty: $0)}, encoding: .utf8)!)
    }
    ...
    func betwixt(_ danumuMsgModel: CastMeasurable?) {
    ...
    @objc func bass(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(bass(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension MeetAdminView: UIGestureRecognizerDelegate {
    ...
    func by(for danumuMsgModel: CastMeasurable?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = HearReactiveCompatible(model: danumuMsgModel)
        ...
        var frame = CGRect(x: user_needValue, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if ReadingLanguageManager.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = user_needValue
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (appTotalroId.lowercased()))
...
//    }
    ...
    func ignoreConnect(_ isFirst: Bool) {
...
extension MeetAdminView {
    // 添加视图
    //: private func setupSubviews() {
    private func personWait() {
