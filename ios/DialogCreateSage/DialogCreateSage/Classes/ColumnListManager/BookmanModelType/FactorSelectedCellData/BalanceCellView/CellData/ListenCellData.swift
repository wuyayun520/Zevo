...
/*: "title" :*/
fileprivate let constSignaturePath:String = "titbirthe"

/*: "content" :*/
fileprivate let kIncreaseName:[Character] = ["c","o","n","t","e","n","t"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let noti_footballTalkHourData:[UInt8] = [0x6,0x1,0x6,0x1b,0x47,0xc,0x0,0xb,0xa,0x1d,0x55,0x46,0x4f,0x7,0xe,0x1c,0x4f,0x1,0x0,0x1b,0x4f,0xd,0xa,0xa,0x1,0x4f,0x6,0x2,0x1f,0x3,0xa,0x2,0xa,0x1,0x1b,0xa,0xb]

private func presentationSeries(exactly num: UInt8) -> UInt8 {
    return num ^ 111
}

/*: "img" :*/
fileprivate let data_appearName:String = "imcling"

/*: "jumpKey" :*/
fileprivate let showCanvasMessage:String = "positive design check decide usedjumpKe"
fileprivate let showCoatId:String = "resolution"
...
@objcMembers public class ListenCellData: BlockCellData {
    // 数据
    //: var extraJson: JSON {
    var extraJson: JSON {
    ...
    public lazy var titleAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.antiquityColor(), .font: UIFont.processAcross(type: .Medium, fontSize: 17)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["title"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(constSignaturePath.replacingOccurrences(of: "birth", with: "l"))].stringValue)
    ...
    public lazy var contentAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.submaxilla(), .font: UIFont.processAcross(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["content"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(String(kIncreaseName))].stringValue)
        //: attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        //: return attributeStr
        return attributeStr
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_footballTalkHourData.map{presentationSeries(exactly: $0)}, encoding: .utf8)!)
    }
    ...
    override public func secretoryPhase() -> CGSize {
        ...
        let height = self.clubMargin(0) - 20
    ...
    override public func clubMargin(_: CGFloat) -> CGFloat {
        ...
        if extraJson[(data_appearName.replacingOccurrences(of: "cling", with: "g"))].stringValue.count > 0 {
        ...
        if extraJson[(constSignaturePath.replacingOccurrences(of: "birth", with: "l"))].stringValue.count > 0 {
        ...
        if extraJson[(String(kIncreaseName))].stringValue.count > 0 {
            //: height += 4
            height += 4
        ...
        if extraJson[(String(showCanvasMessage.suffix(6)) + showCoatId.replacingOccurrences(of: "resolution", with: "y"))].stringValue.count > 0 {
            //: jumpHeight = 35
            jumpHeight = 35
            //: height += jumpHeight
            height += jumpHeight
