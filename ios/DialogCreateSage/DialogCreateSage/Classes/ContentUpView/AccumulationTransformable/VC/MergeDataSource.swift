...
/*: "Search" :*/
fileprivate let appMonthName:[Character] = ["S","e","a","r","c","h"]

/*: "#F5F5F5" :*/
fileprivate let const_alreadyAdjustId:String = "#F5F5F5err mix conscious"

/*: "Search by UID" :*/
fileprivate let user_divisionKey:String = "govern hod lickSearc"

/*: "icon_search_sea" :*/
fileprivate let show_financialOurId:String = "ICON"
fileprivate let kDraftMessage:String = "ch_sgirl mouth product combine"
fileprivate let kSectionLensKey:String = "eplan"

/*: "Can’t find~" :*/
fileprivate let appFortArrayAttractiveFormat:[Character] = ["C","a","n","\u{2019}","t"," ","f","i"]
fileprivate let user_pendingPath:[Character] = ["n","d","~"]

/*: "icon_kong_kong_default" :*/
fileprivate let userExitBeginData:String = "cognitive along somethingicon_"
fileprivate let main_routeFormat:String = "knatureg"
fileprivate let user_backgroundMessage:String = "g_defabove keep"

/*: "Please enter the query UID" :*/
fileprivate let noti_optionName:String = "presentation called secret along marriagePlease"
fileprivate let notiSkinMsg:String = "clean rank positive readinger the"
fileprivate let dataCoalStr:String = "err nobody side closed que"
...
class MergeDataSource: MergeContactViewController {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(appMonthName)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.keyLocal(colors: UIColor.sumerpretation(), size: CGSize(width: 72, height: 32)), for: .normal)
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.placeholder = (String(user_divisionKey.suffix(5)) + "h by UID").localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.parentSet(name: (show_financialOurId.lowercased() + "_sear" + String(kDraftMessage.prefix(4)) + kSectionLensKey.replacingOccurrences(of: "plan", with: "a")))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(FindReactiveCompatible.self, forCellReuseIdentifier: FindReactiveCompatible.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(appFortArrayAttractiveFormat) + String(user_pendingPath)).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(userExitBeginData.suffix(5)) + main_routeFormat.replacingOccurrences(of: "nature", with: "on") + "_kon" + String(user_backgroundMessage.prefix(5)) + "ault")
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + main_buildValue, width: user_needValue, height: kStreamTitle - (14 + main_buildValue + userResolutionUrl)), style: style)
    ...
    private lazy var reqManager: AdminSelectManager = //: return AdminSelectManager()
        .init()
    //: }()
}
...
extension MergeDataSource {
    ...
    private func shut() {
        ...
        guard searchTF.text?.count ?? 0 > 0 else {
            ...
            self.foolishness(showMsg: (String(noti_optionName.suffix(6)) + " ent" + String(notiSkinMsg.suffix(6)) + String(dataCoalStr.suffix(4)) + "ry UID").localized)
            //: return
            return
        }
        ...
        reqManager.someoneIn(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension MergeDataSource: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: FindReactiveCompatible.className(), for: indexPath) as! FindReactiveCompatible
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.total(model: model)
...
extension MergeDataSource: UITextFieldDelegate {
...
extension MergeDataSource: JXPagingViewListViewDelegate {
...
extension MergeDataSource {
    ...
    private func takeADareSubviews() {
    ...
    private func priorityContainer() {
        // 监听搜索框文本长度
