...
fileprivate let mainProviderData:[UInt8] = [0xdf,0xd8,0xdf,0xc2,0x9e,0xd5,0xd9,0xd2,0xd3,0xc4,0x8c,0x9f,0x96,0xde,0xd7,0xc5,0x96,0xd8,0xd9,0xc2,0x96,0xd4,0xd3,0xd3,0xd8,0x96,0xdf,0xdb,0xc6,0xda,0xd3,0xdb,0xd3,0xd8,0xc2,0xd3,0xd2]

private func mobileExactly(draw num: UInt8) -> UInt8 {
    return num ^ 182
}

/*: "icon_rank_left" :*/
fileprivate let notiPortMakeupMessage:[Character] = ["i","c","o","n","_","r","a","n","k","_","l","e","f"]
fileprivate let mainVolumeMsg:String = "past"

/*: "icon_rank_right" :*/
fileprivate let data_tapName:String = "icon_rankprofit represent pretty apply"
fileprivate let data_analysisMsg:String = "_rightnote suite"

/*: "Yesterday" :*/
fileprivate let noti_generateLightFormat:String = "Yestthanks yet during"

/*: "Today" :*/
fileprivate let dataAcceptStr:[Character] = ["T","o","d","a","y"]
...
class RankThen: MergeContactViewController {
    // MARK: params

    //: var titles = [String]()
    var titles = [String]()
    //: var rankPeriodModel = SocialPeriodModel()
    var rankPeriodModel = HappenPeriodModel()
    //: var type = 0
    var type = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainProviderData.map{mobileExactly(draw: $0)}, encoding: .utf8)!)
    }
    ...
    let leftImgView = UIImageView().then {
        ...
        $0.image = UIImage.parentSet(name: (String(notiPortMakeupMessage) + mainVolumeMsg.replacingOccurrences(of: "past", with: "t")))
    }

    //: let rightImgView = UIImageView().then {
    let rightImgView = UIImageView().then {
        //: $0.image = UIImage.BundleImageNamed(name: "icon_rank_right")
        $0.image = UIImage.parentSet(name: (String(data_tapName.prefix(9)) + String(data_analysisMsg.prefix(6))))
    }
...
extension RankThen {
    ...
    private func waitForce(subTitle: String) -> [FactorHandyJSON] {
        //: if subTitle == "Yesterday".localized {
        if subTitle == (String(noti_generateLightFormat.prefix(4)) + "erday").localized {
            //: return addEmptyModel(data: rankPeriodModel.yesterday?.res ?? [])
            return playscript(data: rankPeriodModel.yesterday?.res ?? [])
            //: } else if subTitle == "Today".localized {
        } else if subTitle == (String(dataAcceptStr)).localized {
            //: return addEmptyModel(data: rankPeriodModel.today?.res ?? [])
            return playscript(data: rankPeriodModel.today?.res ?? [])
            //: } else {
        } else {
            //: return addEmptyModel(data: rankPeriodModel.weekly?.res ?? [])
            return playscript(data: rankPeriodModel.weekly?.res ?? [])
        }
    }
    ...
    private func playscript(data: [FactorHandyJSON]) -> [FactorHandyJSON] {
        //: var mData = data
        var mData = data
        //: if mData.count == 0 {
        if mData.count == 0 {
...
extension RankThen: JXSegmentedListContainerViewListDelegate {
...
extension RankThen: JXSegmentedListContainerViewDataSource {
...
extension RankThen {
    ...
    private func draftSubviews() {
        ...
        segmentedDataSource.titleSelectedColor = UIColor.pushDown(207, 72, 255, 1)
        //: segmentedDataSource.titleNormalFont = .pingfangFont(type: .Medium, fontSize: 15)
        segmentedDataSource.titleNormalFont = .processAcross(type: .Medium, fontSize: 15)
        //: segmentedDataSource.titleSelectedFont = .pingfangFont(type: .Medium, fontSize: 15)
        segmentedDataSource.titleSelectedFont = .processAcross(type: .Medium, fontSize: 15)
        ...
        segmentedView.backgroundColor = UIColor.pushDown(212, 131, 255, 1)
        //: segmentedView.defaultSelectedIndex = 1
        segmentedView.defaultSelectedIndex = 1

        //: segmentedView.frame = CGRect(x: (ScreenWidth-270)/2, y: 10, width: totalWidth, height: 36)
        segmentedView.frame = CGRect(x: (user_needValue - 270) / 2, y: 10, width: totalWidth, height: 36)
