...
/*: "icon_randow_user_bg" :*/
fileprivate let noti_agencyText:String = "ICON"
fileprivate let constIonData:String = "dog installation design processingow_u"
...
class OnceMoreView: UIView {
    // MARK: - UI Components

    //: private let leftAvatarView = AvatarView()
    private let leftAvatarView = FormatReactiveCompatible()
    //: private let centerAvatarView = AvatarView()
    private let centerAvatarView = FormatReactiveCompatible()
    //: private let rightAvatarView = AvatarView()
    private let rightAvatarView = FormatReactiveCompatible()
    ...
    private func fort() {
        ...
        centerBgView.image = UIImage.parentSet(name: (noti_agencyText.lowercased() + "_rand" + String(constIonData.suffix(4)) + "ser_bg"))
    }

    //: private func updateAvatarDisplay() {
    private func theResume() {
...
extension OnceMoreView {
    // MARK: - Public Methods

    //: func configure(avatars: [String]) {
    func packDown(avatars: [String]) {
...
class FormatReactiveCompatible: UIView {
    //: var imageurl: String? {
    var imageurl: String? {
        //: didSet {
        didSet {
    ...
    private func hideFormatUi() {
        //: imageView.setUrlImage(urlStr: self.imageurl ?? "")
        imageView.agreeFinish(urlStr: self.imageurl ?? "")
    }
}
