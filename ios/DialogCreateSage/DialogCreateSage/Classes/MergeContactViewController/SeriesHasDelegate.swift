...
class SeriesHasDelegate: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: SuccessSignedSum?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.remotePart()
            }
        }
    ...
    func people() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.strengthOffRequest(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.wateringPot()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.antiquityColor(), NSAttributedString.Key.font: UIFont.processAcross(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.strengthOffRequest(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.antiquityColor(), NSAttributedString.Key.font: UIFont.processAcross(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.antiquityColor()
...
extension SeriesHasDelegate: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            FormalSuccessAdvertisingManager.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
...
extension SeriesHasDelegate: UIGestureRecognizerDelegate {
