...
fileprivate let constSupportName:[UInt8] = [0x1c,0x21,0x1c,0x27,0xdb,0x16,0x22,0x17,0x18,0x25,0xed,0xdc,0xd3,0x1b,0x14,0x26,0xd3,0x21,0x22,0x27,0xd3,0x15,0x18,0x18,0x21,0xd3,0x1c,0x20,0x23,0x1f,0x18,0x20,0x18,0x21,0x27,0x18,0x17]

fileprivate func mailHistory(reach num: UInt8) -> UInt8 {
...
class TicketTagView: UIView {
    //: var titles = [LanguageModel]()
    var titles = [ExecuteLanguageModel]()
    //: var seleteCellArray: [LanguageModel] = []
    var seleteCellArray: [ExecuteLanguageModel] = []
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constSupportName.map{mailHistory(reach: $0)}, encoding: .utf8)!)
    }
...
extension TicketTagView: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: PriceTagCell.className(), for: indexPath) as! PriceTagCell
        //: let title = self.titles[indexPath.row]
        let title = self.titles[indexPath.row]
        //: cell.fill(title: title.name ?? "")
        cell.featureLoad(title: title.name ?? "")
        //: return cell
        return cell
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let cell: PriceTagCell = collectionView.cellForItem(at: indexPath) as! PriceTagCell

        //: var model = self.titles[indexPath.row]
        var model = self.titles[indexPath.row]
        //: if  seleteCellArray.contains(where: { $0.equals(compareTo: model)}) {
        if seleteCellArray.contains(where: { $0.m(compareTo: model) }) {
            //: seleteCellArray.removeAll(where: {$0.equals(compareTo: model)})
            seleteCellArray.removeAll(where: { $0.m(compareTo: model) })
...
extension TicketTagView: AddressLayoutDelegate {
...
extension TicketTagView {
    //: func setframe(frame: CGRect) {
    func frameARe(frame _: CGRect) {
        //: collectionView.frame = self.bounds
        collectionView.frame = self.bounds
        //: collectionView.backgroundColor = .white
        collectionView.backgroundColor = .white
        //: collectionView.register(LanguageTagCell.self, forCellWithReuseIdentifier: LanguageTagCell.className())
        collectionView.register(PriceTagCell.self, forCellWithReuseIdentifier: PriceTagCell.className())
