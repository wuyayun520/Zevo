...
fileprivate let userThumbVantageMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "More" :*/
fileprivate let mainInformationStr:String = "Moreappeal representative discount"

/*: "icon_details_arrow" :*/
fileprivate let dataConsciousMessage:String = "task processingicon_"
fileprivate let main_becomeStr:[Character] = ["d","e","t","a","i","l","s","_","a","r","r","o","w"]

/*: "Post" :*/
fileprivate let mainFutureKey:String = "Postfinancial save limit"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let showBoldContent:String = "successfully sweet designbtn_dyna"
fileprivate let userProgressId:[Character] = ["m","i","c","_","s","t","o"]
fileprivate let appAgentName:String = "p_norhere pad"
...
class BeneathView: UITableViewCell {
    //: var userId: String = ""
    var userId: String = ""
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.previousConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: userThumbVantageMsg.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .pushDown(0, 0, 0, 0.4)
    ...
    lazy var moreButton: AfterView = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .processAcross(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(mainInformationStr.prefix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(dataConsciousMessage.suffix(5)) + String(main_becomeStr))), for: .normal)
    ...
    lazy var pushBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(material), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension BeneathView {
    //: private func bindInteraction() {
    private func roundUsedThird() {}
}
...
extension BeneathView {
    ...
    func configUponPossibilityCamera(userModel: ExecMeasurable) {
        ...
        strA = [(String(mainFutureKey.prefix(4))).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.antiquityColor(), UIColor.submaxilla()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.counteraction(fontSize: 18), UIFont.counteraction(fontSize: 16)]
        ...
        attributedString = attributedString.tab(texts: strA, colors: colorA, fonts: fontA)
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: EyeContactMeasurable = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.ground())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    turning(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.ground())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    turning(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.ground())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    turning(btn: thirdImgView)
                }
            }
        }
    }
    ...
    func turning(btn: UIImageView) {
        ...
        let imag = UIImageView(image: UIImage(named: (String(showBoldContent.suffix(8)) + String(userProgressId) + String(appAgentName.prefix(5)))))
...
extension BeneathView {
    ...
    private func previousConstraint() {
        ...
        let height = (user_needValue - actualWidth(w: 15 * 4)) / 3.0
