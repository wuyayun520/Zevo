...
fileprivate let dataArcForeUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#333333" :*/
fileprivate let k_eventData:String = "#sweetsweet3"
fileprivate let user_failConceptPath:String = "wait"

/*: "#999999" :*/
fileprivate let app_learnEyeStr:String = "#9999"
fileprivate let userHouseMsg:[Character] = ["9","9"]

/*: "isCupid" :*/
fileprivate let userThatId:String = "observer black arrowisCupid"

/*: "autoGreetAuth" :*/
fileprivate let mainFaultFormat:String = "autoGrepull single capture attach additional"
fileprivate let dataSignStr:String = "etAuthcognitive info sit successfully mobile"

/*: "isLeaderboardAnonymous" :*/
fileprivate let constRequirePath:[Character] = ["i","s","L","e","a","d","e","r","b","o","a","r","d","A","n","o","n","y","m","o","u","s"]

/*: "isSameGendered" :*/
fileprivate let showStepTitle:String = "isSameGeterms engage option"
fileprivate let noti_analyzeMoveBullTitle:String = "ndhowed"

/*: "type" :*/
fileprivate let kOwnPath:String = "tyfragment"

/*: "value" :*/
fileprivate let main_keySumPath:String = "valuoccur"
...
class DeviseNotifCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(streetClean), name: UIApplication.didBecomeActiveNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dataArcForeUrl.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (k_eventData.replacingOccurrences(of: "sweet", with: "33") + user_failConceptPath.replacingOccurrences(of: "wait", with: "3")))
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var switchView: UISwitch = {
        ...
        swit.addTarget(self, action: #selector(late), for: .valueChanged)
        //: return swit
        return swit
        //: }()
    }()
    ...
    lazy var detailLB: TensionPreviouslyAlignLb = {
        //: let lb = TextAlignLb.init()
        let lb = TensionPreviouslyAlignLb()
        //: lb.setIsTop(isTop: true)
        lb.treetop(isTop: true)
        //: lb.font = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        lb.font = UIFont.processAcross(type: .Regular, fontSize: 15)
        //: lb.textColor = UIColor.init(hex: "#999999")
        lb.textColor = UIColor(hex: (app_learnEyeStr.capitalized + String(userHouseMsg)))
        //: lb.isHidden = true
        lb.isHidden = true
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
...
extension DeviseNotifCell {
    //: func setCellMessage(titile: String, row: Int, isLast: Bool) {
    func report(titile: String, row: Int, isLast: Bool) {
...
//        if titile == NatureTextConvertible.General.rawValue {
//            MeetRouteThen.checkPushNotification { isShow in
...
//        }
    ...
    @objc func late() {
        ...
        if titleBLB.text == GetHashable.Turnon_Notif.rawValue.localized {
        ...
        } else if titleBLB.text == GetHashable.Cupid.rawValue.localized {
            //: setConfig(type: "isCupid", value: self.switchView.isOn)
            very(type: (String(userThatId.suffix(7))), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Automatic.rawValue.localized {
        } else if titleBLB.text == GetHashable.Automatic.rawValue.localized {
            //: setConfig(type: "autoGreetAuth", value: self.switchView.isOn)
            very(type: (String(mainFaultFormat.prefix(7)) + String(dataSignStr.prefix(6))), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Ranking.rawValue.localized {
        } else if titleBLB.text == GetHashable.Ranking.rawValue.localized {
            //: setConfig(type: "isLeaderboardAnonymous", value: self.switchView.isOn)
            very(type: (String(constRequirePath)), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Same_gendered.rawValue.localized {
        } else if titleBLB.text == GetHashable.Same_gendered.rawValue.localized {
            //: setConfig(type: "isSameGendered", value: self.switchView.isOn)
            very(type: (String(showStepTitle.prefix(8)) + noti_analyzeMoveBullTitle.replacingOccurrences(of: "how", with: "er")), value: self.switchView.isOn)
        }
    }

    //: func setConfig(type: String, value: Bool) {
    func very(type: String, value: Bool) {
        //: let va = value ? 1 : -1
        let va = value ? 1 : -1
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["type"] = type
        dict[(kOwnPath.replacingOccurrences(of: "fragment", with: "pe"))] = type
        //: dict["value"] = va
        dict[(main_keySumPath.replacingOccurrences(of: "occur", with: "e"))] = va

        //: ProgressHUD.show()
        PointerReactiveCompatible.soundAssetThrow()
        //: TalkingMeRequestTool.req_SettingChange(params: dict) { succeed, result, errorModel in
        GreatThen.toALesserExtent(params: dict) { succeed, _, _ in
            //: ProgressHUD.dismiss()
    ...
    @objc func streetClean(notification _: NSNotification) {
//        if titleBLB.text == NatureTextConvertible.General.rawValue {
//            MeetRouteThen.checkPushNotification { isShow in
//                DispatchQueue.main.sync {
//                    self.switchView.isOn = isShow
//                }
