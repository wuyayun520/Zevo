...
fileprivate let mainSpringMsg:[UInt8] = [0x7,0x0,0x7,0x1a,0x46,0xd,0x1,0xa,0xb,0x1c,0x54,0x47,0x4e,0x6,0xf,0x1d,0x4e,0x0,0x1,0x1a,0x4e,0xc,0xb,0xb,0x0,0x4e,0x7,0x3,0x1e,0x2,0xb,0x3,0xb,0x0,0x1a,0xb,0xa]

private func saleVisitor(multi num: UInt8) -> UInt8 {
    return num ^ 110
}

/*: "img_loading_lose" :*/
fileprivate let k_guideUrl:[Character] = ["i","m","g","_","l","o"]
fileprivate let showClipTradeTitle:String = "factor estate capacity variety mmading_"
...
class CandleFlamePhotosCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainSpringMsg.map{saleVisitor(multi: $0)}, encoding: .utf8)!)
    }
...
extension CandleFlamePhotosCell {
    //: private func bindInteraction() {
    private func commit() {
        //: let singleTap = UITapGestureRecognizer(target: self, action: #selector(singleTapAction))
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(sweepingAction))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(perimeter)).then {
...
extension CandleFlamePhotosCell {
    ...
    func aboveMention(imgModel: HypothesisModelType) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(k_guideUrl) + String(showClipTradeTitle.suffix(6)) + "lose")), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.stigmatise()
            }
        }
    ...
    func reclaimBecome(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(k_guideUrl) + String(showClipTradeTitle.suffix(6)) + "lose")), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.stigmatise()
        }
    }
...
extension CandleFlamePhotosCell: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension CandleFlamePhotosCell {
    ...
    func stigmatise() {
        ...
        var newW = user_needValue
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = user_needValue * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > kStreamTitle {
            //: newH = ScreenHeight
            newH = kStreamTitle
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * kStreamTitle / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (user_needValue - newW) / 2.0, y: (kStreamTitle - newH) / 2)
