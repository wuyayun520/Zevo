...
fileprivate let noti_seatPath:String = "icon_recommend ad close positive glass"
fileprivate let show_writingUrl:String = "zone exclusive resumekong_k"
fileprivate let notiRunningMsg:String = "perfault"

/*: "page" :*/
fileprivate let dataPingKey:[UInt8] = [0x65,0x67,0x61,0x70]

/*: "uid" :*/
fileprivate let noti_donFormat:[UInt8] = [0xfa,0xe6,0xeb]

private func engageContext(match num: UInt8) -> UInt8 {
    return num ^ 143
}

/*: "list" :*/
fileprivate let userSinkThumbStr:String = "LIST"
...
class BlockRecognizerDelegate: MergeContactViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [EyeContactMeasurable]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: constHalfScreenPath, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(ColumnReactiveCompatible.self, forCellWithReuseIdentifier: ColumnReactiveCompatible.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.parentSet(name: (String(noti_seatPath.prefix(5)) + String(show_writingUrl.suffix(6)) + "ong_" + notiRunningMsg.replacingOccurrences(of: "per", with: "de")))
...
extension BlockRecognizerDelegate {
    ...
    private func timingData() {
        //: let params = ["page": pageIndex, "uid": userInfoModel.uid] as [String: Any]
        let params = [String(bytes: dataPingKey.reversed(), encoding: .utf8)!: pageIndex, String(bytes: noti_donFormat.map{engageContext(match: $0)}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        ContentWeeklyReactiveCompatible.confirmAnimal(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(userSinkThumbStr.lowercased())] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = LivingMomentModel.deserialize(from: dict as? Dictionary) {
...
extension BlockRecognizerDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ColumnReactiveCompatible.className(), for: indexPath) as! ColumnReactiveCompatible
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = SceneReactiveCompatible(model: detailModel)
...
extension BlockRecognizerDelegate: JXPagingViewListViewDelegate {
...
extension BlockRecognizerDelegate {
    ...
    func commentEyeController(_ userModel: ExecMeasurable) {
