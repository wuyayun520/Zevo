...
fileprivate let app_gloryHairCostUrl:[UInt8] = [0x9a,0x9d,0x9a,0x87,0xdb,0x90,0x9c,0x97,0x96,0x81,0xc9,0xda,0xd3,0x9b,0x92,0x80,0xd3,0x9d,0x9c,0x87,0xd3,0x91,0x96,0x96,0x9d,0xd3,0x9a,0x9e,0x83,0x9f,0x96,0x9e,0x96,0x9d,0x87,0x96,0x97]

private func fireControl(suspend num: UInt8) -> UInt8 {
    return num ^ 243
}

/*: "gift_bg_dm" :*/
fileprivate let dataOuterName:String = "gift_bgfire certain tick"
fileprivate let notiUnlessValue:String = "_dmpositive priority enable"

/*: "#FFF4B5" :*/
fileprivate let showTechnicalTitle:String = "bring mask route via pause#F"
fileprivate let notiTurkeyTitle:String = "FFproximateB5"

/*: "won " :*/
fileprivate let constServeId:String = "distant streamwon "

/*: " coins in " :*/
fileprivate let userLabStr:String = "express meeting coin"

/*: "just sent" :*/
fileprivate let notiBrowWeatherBeFormat:[Character] = ["j","u","s","t"," "]
fileprivate let const_loopId:String = "smakeupnt"

/*: "#4120C1" :*/
fileprivate let dataLocationName:String = "#4120C1depict difference"
...
class FinishView: UIView {
    ...
    private var giftModel = CandidModelType()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_gloryHairCostUrl.map{fireControl(suspend: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.parentSet(name: (String(dataOuterName.prefix(7)) + String(notiUnlessValue.prefix(3))))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(showTechnicalTitle.suffix(2)) + notiTurkeyTitle.replacingOccurrences(of: "proximate", with: "4")))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(showTechnicalTitle.suffix(2)) + notiTurkeyTitle.replacingOccurrences(of: "proximate", with: "4")))
...
extension FinishView {
    ...
    func angle(_ model: CandidModelType) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.agreeFinish(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(constServeId.suffix(4))).localized + "\(giftModel.totalCoin)" + (String(userLabStr.suffix(5)) + "s in ").localized + giftModel.gameName
            ...
            giftImgV.agreeFinish(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (String(notiBrowWeatherBeFormat) + const_loopId.replacingOccurrences(of: "makeup", with: "e")).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = giftmodelDetect(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(dataLocationName.prefix(7))))!, range: nsRange)
            }
    ...
    private func giftmodelDetect(giftmodel: CandidModelType) -> String {
...
extension FinishView {
    ...
    private func noteSubviews() {
        //: self.frame = CGRect(x: 0, y: StatusBarNavigationBarHeight, width: ScreenWidth, height: 60)
        self.frame = CGRect(x: 0, y: userResolutionUrl, width: user_needValue, height: 60)
