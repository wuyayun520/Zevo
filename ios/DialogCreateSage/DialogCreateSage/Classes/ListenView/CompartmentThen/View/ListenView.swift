...
fileprivate let show_layerMsg:[UInt8] = [0x53,0x58,0x53,0x5e,0x12,0x4d,0x59,0x4e,0x4f,0x5c,0x24,0x13,0xa,0x52,0x4b,0x5d,0xa,0x58,0x59,0x5e,0xa,0x4c,0x4f,0x4f,0x58,0xa,0x53,0x57,0x5a,0x56,0x4f,0x57,0x4f,0x58,0x5e,0x4f,0x4e]

fileprivate func passJudge(bank num: UInt8) -> UInt8 {
...
enum SweetSwipeDirection: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol MilitaryPolicemanCellDelegete: NSObjectProtocol {
    ///
...
class ListenView: UIView {
    ...
    var cell_delegate: MilitaryPolicemanCellDelegete?

    //: var model: TalkingMatchResultModel?
    var model: RoundingModelType?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_layerMsg.map{passJudge(bank: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(crosswise(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension ListenView: UIGestureRecognizerDelegate {
    ...
    @objc func crosswise(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if ReadingLanguageManager.shared.direction == .rightToLeft {
                ...
                if movePoint.x > user_needValue / 4 {
                    ...
                    self.cell_delegate?.harbour(cell: self, forMovePoint: self.currentPoint)
    ...
    func seekSuperview() {
        ...
        self.cell_delegate?.estimatedCell(cell: self)
    }
    ...
    func arrowSecond(direction: SweetSwipeDirection) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
