...
/*: "/TXCache" :*/
fileprivate let k_policyUrl:String = "apply hung aspect girl collect/TXCache"
...
protocol TalkingOptDelegateReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func span(player: CapReactiveCompatible, status: FacePlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func followingMp(player: CapReactiveCompatible, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func funcWithinProgress(player: CapReactiveCompatible, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class CapReactiveCompatible: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: TalkingOptDelegateReactiveCompatible?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: FacePlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = CapReactiveCompatible()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(k_policyUrl.suffix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(library), userInfo: nil, repeats: true)
...
extension CapReactiveCompatible {
    ...
    func glitter(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: userDismissUrl.allTitle(), with: userDeviceIndexStr)
    ...
    @objc func library() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.funcWithinProgress(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func margin(renderMode: FinishEvenRenderMode) {
    ...
    func doinglyTurn(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isStrategy == true {
        if FileSocketManager.shared.isStrategy == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func headboard() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.span(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func existPlayings() -> Bool {
...
extension CapReactiveCompatible: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.followingMp(player: self, duration: Int(duration), currentTime: Int(currentTime))
