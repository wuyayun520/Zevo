...
fileprivate let dataAnimaName:[UInt8] = [0xf3,0xf4,0xf3,0xee,0xb2,0xf9,0xf5,0xfe,0xff,0xe8,0xa0,0xb3,0xba,0xf2,0xfb,0xe9,0xba,0xf4,0xf5,0xee,0xba,0xf8,0xff,0xff,0xf4,0xba,0xf3,0xf7,0xea,0xf6,0xff,0xf7,0xff,0xf4,0xee,0xff,0xfe]

/*: "F5F5F5" :*/
fileprivate let const_governMsg:String = "f5f5f5"
...
class FactorReactiveCompatible: UITableViewCell {
    ...
    var videoView = CellReactiveCompatible()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataAnimaName.map{$0^154}, encoding: .utf8)!)
    }
    ...
    func towardTableEnd(isListTableCell: Bool) {
    ...
    lazy var userInfoView: RecoverBroadThen = {
        //: let  view = TalkingMomentUserInfoView.init()
        let view = RecoverBroadThen()
    ...
    lazy var textContentView: AuthorAssemblageReactiveCompatible = {
        //: let  view = TalkingMomentTextContentView.init()
        let view = AuthorAssemblageReactiveCompatible()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var photoView: CastDataSource = {
        ...
        let view = CastDataSource(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: MeetReaderThen = {
        //: let  view = TalkingMomentItemBottomView.init()
        let view = MeetReaderThen()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (const_governMsg.uppercased()))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
extension FactorReactiveCompatible {
    //: func configCell(model: TalkingMomentModel) {
    func compute(model: LivingMomentModel) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.homegrown(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.frontAnima(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == MpBidirectionalCollection.Photo.rawValue || model.momentType == MpBidirectionalCollection.Live.rawValue {
...
//            }
            ...
            photoView.outputModel(model: cunrrenModel)
...
//            }
            ...
            videoView.direction(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.decision(model: cunrrenModel)
...
//        }
    ...
    @objc func informing() {
        ...
        let vc = HostViewController(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != CompositionReactiveCompatible.share.loginUserMode.sex {
        if cunrrenModel.sex != CompositionReactiveCompatible.share.loginUserMode.sex {
    ...
    func picture() {
        ...
        if cunrrenModel.momentType == MpBidirectionalCollection.Video.rawValue {
            //: videoView.setupPlayer()
            videoView.lengthProcess()
        }
    }
...
extension FactorReactiveCompatible {
    ...
    private func smooth() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(informing))
