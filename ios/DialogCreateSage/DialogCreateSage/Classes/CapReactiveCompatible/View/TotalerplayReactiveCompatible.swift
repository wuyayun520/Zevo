...
fileprivate let data_depictFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let constPunishData:String = "pro through outer via calledbtn_dy"
fileprivate let main_resentValue:String = "response bank estate_stop_no"
fileprivate let appFirmIsolateStr:[Character] = ["r"]

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let data_teamFormat:String = "btyouw"
fileprivate let data_accountingStr:String = "along successfully concept small sizeh_tdj"
fileprivate let constConstraintFormReportUrl:String = "locationop"

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let constAssessmentFormat:String = "voice security drop permissionbtnC"
fileprivate let showPunishFormat:[Character] = ["j","_","o","e","d","i","v","_","p","l","a","y"]
...
// __CLOSE_PRINT__
//
//  TotalerplayReactiveCompatible.swift
//  AbroadTalking
//
...
protocol ExecViewDelegate: NSObject {
    //: func func__interactionViewStatus(view: TalkingVideoPlayerInteractionView, status: InteractionViewStatus)
    func termsStatus(view: TotalerplayReactiveCompatible, status: TrackViewStatus)

    //: func func__interactionViewSlideValue(view: TalkingVideoPlayerInteractionView, value: CGFloat)
    func quantityerchange(view: TotalerplayReactiveCompatible, value: CGFloat)
}

//: class TalkingVideoPlayerInteractionView: UIView {
class TotalerplayReactiveCompatible: UIView {
    //: open weak var delegate: InteractionViewDelegate?
    open weak var delegate: ExecViewDelegate?
    ...
    override init(frame: CGRect) {
        ...
        self.convertWillMagnitudeeraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: data_depictFormat.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        ...
        btn.setImage(UIImage.parentSet(name: (String(constPunishData.suffix(6)) + "namic" + String(main_resentValue.suffix(8)) + String(appFirmIsolateStr))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(blindUsed(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        ...
        btn.setImage(UIImage.parentSet(name: (data_teamFormat.replacingOccurrences(of: "you", with: "n") + "OblE" + String(data_accountingStr.suffix(5)) + "_oediv_s" + constConstraintFormReportUrl.replacingOccurrences(of: "location", with: "t"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.parentSet(name: (String(constAssessmentFormat.suffix(4)) + "mjoS7_td" + String(showPunishFormat))), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(clickSender(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var failImageView: UIImageView = {
        ...
        imag.image = UIImage.parentSet(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: ExampleReactiveCompatible = {
        ...
        let slid = ExampleReactiveCompatible(frame: CGRect(x: 100, y: 0, width: user_needValue - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension TotalerplayReactiveCompatible {
    //: @objc func centerButtonClickAction(sender: UIButton) {
    @objc func blindUsed(sender: UIButton) {
        //: sender.isHidden = true
        sender.isHidden = true
        //: self.handleStatus(status: .StartPlay)
        self.passRecognize(status: .StartPlay)
    }
...
extension TotalerplayReactiveCompatible {
    ...
    private func convertWillMagnitudeeraction() {
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.quantityerchange(view: self, value: value)
        }
        ...
        self.slider.panEndHandler = { [weak self] _ in
            ...
            self.passRecognize(status: .StopSeek)
        }
    }
}
