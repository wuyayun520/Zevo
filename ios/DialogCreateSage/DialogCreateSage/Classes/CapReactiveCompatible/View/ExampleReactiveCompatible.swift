...
fileprivate let mainCoalNobodyCloudKey:[UInt8] = [0x8f,0x94,0x8f,0x9a,0x4e,0x89,0x95,0x8a,0x8b,0x98,0x60,0x4f,0x46,0x8e,0x87,0x99,0x46,0x94,0x95,0x9a,0x46,0x88,0x8b,0x8b,0x94,0x46,0x8f,0x93,0x96,0x92,0x8b,0x93,0x8b,0x94,0x9a,0x8b,0x8a]

fileprivate func notePing(skirt num: UInt8) -> UInt8 {
...
// __CLOSE_PRINT__
//
//  ExampleReactiveCompatible.swift
//  AbroadTalking
//
...
class ExampleReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainCoalNobodyCloudKey.map{notePing(skirt: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: OverThen = {
        ...
        let v = OverThen(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(busy(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(mid(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension ExampleReactiveCompatible {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func busy(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func mid(gesture: UIPanGestureRecognizer) {
