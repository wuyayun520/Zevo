...
fileprivate let appSneakKey:[UInt8] = [0xc5,0xc2,0xc5,0xd8,0x84,0xcf,0xc3,0xc8,0xc9,0xde,0x96,0x85,0x8c,0xc4,0xcd,0xdf,0x8c,0xc2,0xc3,0xd8,0x8c,0xce,0xc9,0xc9,0xc2,0x8c,0xc5,0xc1,0xdc,0xc0,0xc9,0xc1,0xc9,0xc2,0xd8,0xc9,0xc8]

/*: "#4FAAFF" :*/
fileprivate let appShotName:String = "mouth that personality every#4FAAFF"

/*: "reply" :*/
fileprivate let show_normalContent:[Character] = ["r","e","p","l","y"]

/*: "F5F5F5" :*/
fileprivate let userDetailedId:String = "bucket5bucket5bucket5"
...
class AuthorView: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appSneakKey.map{$0^172}, encoding: .utf8)!)
    }
...
extension AuthorView {
    ...
    @objc func along(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension AuthorView {
    //: func configCell(model: TalkingCommentReplyModel) {
    func bot(model: MpReplyModel) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(appShotName.suffix(7))))
            ...
            let font = UIFont.joinSize(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (String(show_normalContent)).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.pointOfTotalersection(), UIColor.antiquityColor(), UIColor.pointOfTotalersection(), UIColor.antiquityColor()]
                ...
                colorA = [UIColor.pointOfTotalersection(), UIColor.antiquityColor()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func passUser(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: user_needValue - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (userDetailedId.replacingOccurrences(of: "bucket", with: "F")))?.cgColor
...
extension AuthorView {
    ...
    private func patternAlbum() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(along(sender:)))
