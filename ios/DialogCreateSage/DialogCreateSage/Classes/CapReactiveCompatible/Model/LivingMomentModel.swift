...
struct LivingMomentModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var pic = [HostEvenPhotoModel]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: ReminderTalkingVideoModel?
...
struct HostEvenPhotoModel: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum MpBidirectionalCollection: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct ReminderTalkingVideoModel: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: VisualCommunicationNameConvertible?
}

///
//: enum MomentVideoStatus: String {
enum VisualCommunicationNameConvertible: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension LivingMomentModel {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func activity(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(SizeThen.sizePrompt(text: content!, size: CGSize(width: user_needValue - 54, height: CGFloat(MAXFLOAT)), font: UIFont.processAcross(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == MpBidirectionalCollection.Photo.rawValue || momentType == MpBidirectionalCollection.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
