...
/*: "%@ reply %@：%@" :*/
fileprivate let notiChannelContent:[Character] = ["%","@"," ","r","e","p","l","y"]
fileprivate let noti_rejectName:[Character] = [" ","%","@","：","%","@"]
...
extension MpReplyModel {
    ...
    mutating func decide() {
        ...
        if replyUser.count > 0 {
            ...
            tempContent = String(format: (String(notiChannelContent) + String(noti_rejectName)), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(SizeThen.sizePrompt(text: tempContent, size: CGSize(width: user_needValue - 82, height: CGFloat(MAXFLOAT)), font: UIFont.processAcross(type: .Regular, fontSize: 14), lineNumber: 0).height)
