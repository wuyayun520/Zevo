...
/*: "reply" :*/
fileprivate let mainSplitText:[Character] = ["r","e","p","l","y"]
...
struct OverMeasurable: HandyJSON {
    //: var cid: String?
...
extension OverMeasurable {
    //: mutating func caculateCommentItemHeight() {
    mutating func bindMerge() {
        //: var tempContent = ""
        var tempContent = ""
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if ReadingLanguageManager.shared.direction == .leftToRight {
                ...
                tempContent = (String(mainSplitText)).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (String(mainSplitText)).localized
            }
        ...
        let sizeheight = Int(SizeThen.sizePrompt(text: tempContent, size: CGSize(width: user_needValue - 70, height: CGFloat(MAXFLOAT)), font: UIFont.processAcross(type: .Regular, fontSize: 16), lineNumber: 0).height)
